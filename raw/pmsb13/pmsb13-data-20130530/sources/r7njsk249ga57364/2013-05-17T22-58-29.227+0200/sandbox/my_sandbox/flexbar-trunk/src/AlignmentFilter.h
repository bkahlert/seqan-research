...
public:
	...
	int align(void* item, const bool performRemoval){
		...
		int fmismatches, fgapsR, fgapsA, foverlapLength, ftailLength;
		...
		if(qIndex >= 0){
			...
			if(m_verb == ALL || (m_verb == MOD && performRemoval)){
				...
				ss << "  query tag        " << m_queries->at(qIndex).first->getSequenceTag() << "\n"
				   << "  read tag         " << readTag                       << "\n"
				   << "  read             " << read                          << "\n"
				...
				ss << readTag     << "\t" << m_queries->at(qIndex).first->getSequenceTag() << "\t"
