...
void defineOptionsAndHelp(seqan::ArgumentParser &parser, const std::string version, const std::string date){
	...
	addOption(parser, ArgParseOption("t", "target", "Prefix for output file names.", ArgParseArgument::OUTPUTFILE));
	...
	addOption(parser, ArgParseOption("bt", "barcode-threshold", "Allowed mismatches and indels per 10 bases overlap.", ArgParseArgument::DOUBLE));
	...
	addOption(parser, ArgParseOption("ad", "adapter-read-set", "Restriction to read set in paired mode.", ArgParseArgument::STRING));
	addOption(parser, ArgParseOption("ao", "adapter-min-overlap", "Minimum overlap of adapter and read.", ArgParseArgument::INTEGER));
	addOption(parser, ArgParseOption("at", "adapter-threshold", "Allowed mismatches and indels per 10 bases overlap.", ArgParseArgument::DOUBLE));
	...
	addText(parser._toolDoc, "RIGHT_TAIL: use only last n bases, see tail-length options", false);
	 ...
	 setRequired(parser, "format");
	 setRequired(parser, "reads");
	// 
	 setMinValue(parser, "threads", "1");
	 setValidValues(parser, "format", "csfasta csfastq fasta fastq fastq-sanger fastq-solexa fastq-i1.3 fastq-i1.5 fastq-i1.8");
	...
	setMinValue(parser, "max-uncalled",     "0");
	 ...
	 setMinValue(parser, "min-readlength",   "1");
...
struct Options{
	...
	std::string readsFile, readsFile2, barReadsFile, barcodeFile, adapterFile;
	std::string adapterSeq, targetName, log_level;
...
void loadProgramOptions(Options &o, seqan::ArgumentParser &parser){
	...
	else if(format == "fastq-i1.3"){
		o.format = FASTQ;
		o.qual   = ILLUMINA13;
	}
	else if(format == "fastq-i1.5"){
		o.format = FASTQ;
		o.qual   = ILLUMINA13;
	}
	else if(format == "fastq-i1.8"){
		o.format = FASTQ;
		o.qual   = SANGER;
	}
	...
	if(isSet(parser, "fasta-output") && (o.format == FASTQ || o.format == CSFASTQ)){
		o.switch2Fasta = true;
		
		if(o.format == FASTQ)   o.format = FASTA;
		if(o.format == CSFASTQ) o.format = CSFASTA;
	}
	...
	if(o.adapRm != AOFF){
		...
		cout << "adapter-trim-end:      " << a_trim_end << endl;
		...
		if(isSet(parser, "adapter-read-set") &&
		   (o.runType == PAIRED || o.runType == PAIRED_BARCODED)){
			
			string a_read_set;
			getOptionValue(a_read_set, parser, "adapter-read-set");
			cout << "adapter-read-set:      " << a_read_set << endl;
