...
public:
	...
	void align(const TString &querySeq, const TString &read, int &gapsR, int &gapsA, int &mismatches, int &startPos, int &endPos, int &startPosA, int &endPosA, int &startPosS, int &endPosS, int &aliScore, std::stringstream &aliString, TString &tagSeq){
		...
		if(m_trimEnd == flexbar::RIGHT || m_trimEnd == flexbar::RIGHT_TAIL){
			AlignConfig<true, false, true, true> ac;
			if(m_isColorSpace) aliScore = globalAlignment(align, m_score,     ac, NeedlemanWunsch());
			else               aliScore = globalAlignment(align, m_scoreDna5, ac, NeedlemanWunsch());
		}
		else if(m_trimEnd == flexbar::LEFT  || m_trimEnd == flexbar::LEFT_TAIL){
		...
		// globalAlignment(align, m_scoreDna5, ac, 2, 2, MyersHirschberg());
