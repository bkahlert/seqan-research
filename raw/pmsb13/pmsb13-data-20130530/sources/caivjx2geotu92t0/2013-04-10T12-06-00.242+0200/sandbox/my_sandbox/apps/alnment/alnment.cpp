...
int main() {
    ...
    {	
        ...
        std::cout << align3;
		++k;
        std::cout << "Aligns Seq1[" << clippedBeginPosition(row(align3, 0)) << ":" << (clippedEndPosition(row(align3, 0))-1) << "]";
        std::cout << " and Seq2[" << clippedBeginPosition(row(align3, 1)) << ":" <<  (clippedEndPosition(row(align3, 1))-1) << "]" << std::endl << std::endl;
    }
