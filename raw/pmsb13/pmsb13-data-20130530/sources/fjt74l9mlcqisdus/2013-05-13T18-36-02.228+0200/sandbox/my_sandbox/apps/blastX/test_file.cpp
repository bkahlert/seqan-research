...
SEQAN_DEFINE_TEST(test_my_app_get_alphabet)
{
	...
	unsigned diffLettersLen;
	StringSet<String<AminoAcid> > alphabets;
	String<unsigned int> numb_alp = "1234567";
	for(unsigned size_alp = 6; size_alp < 21; ++size_alp)
	{
		alphabets = GET_ALPHABET(size_alp, numb_alp);
		for(unsigned alphabet = 0; alphabet < length(alphabets); ++alphabet)		
		{	
			append(diffLetters, alphabets[alphabet][0]);
			for(unsigned i = 1; i < length(alphabets[alphabet]); ++i)
			{
				diffLettersLen = length(diffLetters);
				for(unsigned j = 0; j < diffLettersLen; ++j)
				{
					if(alphabets[alphabet][i] != diffLetters[j])
