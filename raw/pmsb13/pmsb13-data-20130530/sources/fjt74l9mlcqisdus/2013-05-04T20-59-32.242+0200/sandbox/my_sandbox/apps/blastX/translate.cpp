...
int translate_reads(StrSetSA & trans_reads,String<Dna> & read,String<AminoAcid> & Alphabete,int frame){
...
int get_translate_for_all(StringSet<String<Dna>> & Reads,StrSetSA & trans_reads,String<AminoAcid> & Alphabete){	
// Schleife fuer jedes Read // Uebersetzung
	for (int read=0;read<length(Reads);++read){
		...
		for(int reading_frame=0;reading_frame<6;++reading_frame){
			...
			if (translate_reads(trans_reads,Reads[read],Alphabete,reading_frame)==1){
...
int translate_datenbank(StrSetSA & trans_proteine,StrSetSA & Proteine,String<AminoAcid> & Alphabete){
