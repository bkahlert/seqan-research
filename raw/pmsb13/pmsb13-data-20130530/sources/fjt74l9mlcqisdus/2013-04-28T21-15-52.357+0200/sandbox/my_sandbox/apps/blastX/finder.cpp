...
// 
void FIND_MATCHES(StringSet<String<Dna5>> & Reads,StringSet<String<char>> & ReadID,StrSetSA & Proteine,
StringSet<String<char>> & ProteinID, int seed,StrSetSA & Alphabete){
	...
	for(int alp=0;alp<length(Alphabete);++alp){
		...
		for (int p=0;p<length(trans_reads);p++) cout << trans_reads[p]<<"\t";
		cout<<endl;
		...
		for (int p=0;p<length(trans_proteine);p++) cout << trans_proteine[p]<<"\t";
		cout <<endl;
		
		
		Index <StrSetSA> my_index(trans_proteine);
		Finder <Index<StrSetSA>> finder(my_index);
		for (int o=0;o<length(trans_reads);++o){
			//Pattern <String<int>> pattern(trans_reads[o]);
			while(find(finder,trans_reads[o])){
