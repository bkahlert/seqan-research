...
StringSet<String<char> > & ProteinID, int seed,StrSetSA & Alphabete){
	...
	for(int alp=0;alp<length(Alphabete);++alp){
		...
		*/
		...
		Finder <Index<StrSetSA>,Backtracking<HammingDistance> > finder_obj(my_index);
		Pattern <String<AminoAcid>,Backtracking<HammingDistance> > pattern_obj(index_trans_reads,length(trans_reads));
	    
		while(find(finder_obj,pattern_obj)){
