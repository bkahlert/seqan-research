...
StringSet<String<AminoAcid> > GET_ALPHABET_FORCE()
{
	...
	*@return Gibt einen String mit integer zurueck
	*/
...
String<int> GET_ALPHABET(unsigned & alpGroup)
{
	/**!@brief GET_ALPHABET gibt ein reduziertes Aminosaeurealphabet zurueck.
	*Alphabet entseht durch Clustern mit zur Hilfenahme einer Scorematrix (z.B. Blosum62, PAM120, etc..)
	...
	*/
	...
	Score<int, ScoreMatrix<AminoAcid, Blosum45_> > scoreOfMatrix;
	...
	for(unsigned r = 0; r < run; ++r)
	{
		...
		//Setzt den score auf -50 damit er beim naechsten Durchgang nicht wieder gefunden wird			
