...
StringSet<String<char> > & ProteinID, int & seed,int & distance,StrSetSA & Alphabete){
	...
	for(int alp=0;alp<length(Alphabete);++alp){
		...
		if (translate_reads(Reads,trans_reads,Alphabete[alp])) return 1;
		...
		if (verify_all(seed_found,distance,trans_proteine,trans_reads,verify_found,length(Reads[0]))) return 1:
