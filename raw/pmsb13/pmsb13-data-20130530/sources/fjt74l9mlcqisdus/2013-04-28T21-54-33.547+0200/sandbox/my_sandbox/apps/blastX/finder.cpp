...
StringSet<String<char>> & ProteinID, int seed,StrSetSA & Alphabete){
	...
	for(int alp=0;alp<length(Alphabete);++alp){
		...
		Finder <Index<StrSetSA>,Backtracking<HammingDistance>> finder(my_index,0);
		Pattern <Index<StrSetSA>,Backtracking<HammingDistance>> my_pattern(my_index2,0);
		//find(finder,my_pattern);
		//while(find(finder,my_pattern)){
	//		cout<<"ok";
