...
*/	
int known_position(int & begin,int & end,StringSet<Pair<unsigned int, unsigned int>> & known_position){
	for (int i=0;i<length(known_position);++i){
		if (known_position[i].i1 == begin && known_position[i].i2 == end) return 0;
	}
	return 1;
}
...
int verify_all(Match_found & seed_found, int & distance,StrSetSA & trans_proteine,StrSetSA & trans_reads,Match_found & verify_found,StringSet<String<Dna> > & Reads,StrSetSA & Proteine){
	...
	int read_number = 0;
	do{
		StringSet<Pair<unsigned int, unsigned int>> known_position;
		for (int i=0;i<length(seed_found.position_read);++i){
			if (read_number == seed_found.position_read[i]){
				...
				if (begin>=0 && end<=length(trans_proteine[seed_found.position_protein[i]]) && known_position(begin,end,known_position)){
					Pair<unsigned int,unsigned int> new_position (begin,end);
					appendValue(known_position,new_position);
