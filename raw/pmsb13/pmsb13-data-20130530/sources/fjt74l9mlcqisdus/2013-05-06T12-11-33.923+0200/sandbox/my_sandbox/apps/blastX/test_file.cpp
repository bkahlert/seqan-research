...
SEQAN_DEFINE_TEST(test_my_app_find_matches)
{
	...
	SEQAN_ASSERT(length(test.position_read)==length(test.begin_read) && length(test.begin_read)==length(test.end_read) &&
	length(test.end_read) == length(test.position_protein) && length(test.position_protein)==length(test.begin_protein) &&
