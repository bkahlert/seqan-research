...
SEQAN_DEFINE_TEST(test_my_app_append_to_match_found)
{
	...
	append_to_match_found(test,x,y,z,3,10);
...
SEQAN_DEFINE_TEST(test_my_app_find_matches)
{
	...
	int result = find_matches(trans_proteine,trans_reads,seed,0,test0);
	SEQAN_ASSERT_EQ(result,0);
	...
	Match_found test;
	result = find_matches(trans_proteine,trans_reads,3,test);
	SEQAN_ASSERT(length(test.position_read)==length(test.begin_read) && length(test.end_read)==length(test.position_protein)
		&& length(test.begin_protein)==length(test.end_protein) &&length(test.position_read)==length(test.position_protein)
		&& length(test.end_protein)==length(test.position_read));
}
// TEST APPEND_TO_FIND_MATCHES -----------------------------------------------------------------------------------------
