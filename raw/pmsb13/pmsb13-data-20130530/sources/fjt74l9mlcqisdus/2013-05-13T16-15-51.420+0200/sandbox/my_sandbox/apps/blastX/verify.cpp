...
int verify_seed_match(String<AminoAcid> protein,String<AminoAcid> & read,Match_found & verify_found,int & begin, int & end, int position_protein,int & position_read){
		...
		for (int i=0;i<length(seed_found.position_read);++i){
			if (read_number == seed_found.position_read[i]){
				...
				if (begin>=0 && end<=length(trans_proteine[seed_found.position_protein[i]]) && known_position(begin,end,pair_position)){
					...
					verify_seed_match(infix(trans_proteine[seed_found.position_protein[i]],begin,end),trans_reads[read_number],verify_found,begin,end,seed_found.position_protein[i],read_number);
				}
			}	
		}
		read_number ++;
