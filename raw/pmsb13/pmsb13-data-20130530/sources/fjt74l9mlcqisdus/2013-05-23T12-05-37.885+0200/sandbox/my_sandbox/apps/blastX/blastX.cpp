...
int main(int argc, char const ** argv){
    ...
    for(unsigned alp=0;alp<length(alphabets);++alp){
		...
		position_read; /**< Position des Reads im StringSet der uebersetzten Reads */
		
		reserve(seed_found.position_read,100000);
		reserve(seed_found.begin_read,100000);
		reserve(seed_found.end_read,100000);
		reserve(seed_found.position_protein,100000);
		reserve(seed_found.begin_protein,100000);
		reserve(seed_found.end_protein,100000);
		if (FIND_MATCHES(trans_proteine,trans_reads,comVal.seed,comVal.hamming_distance,seed_found)) return 1;
		cout << "FIND_MATCHES_"<<alp<<endl;
		...
		if (VERIFY_ALL(seed_found,comVal.hamming_distance,trans_proteine,real_trans_reads,verify_found,proteine)) return 1;
