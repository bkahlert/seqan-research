...
SEQAN_DEFINE_TEST(test_my_app_find_matches)
{
	...
	Match_found test;
	int result = find_matches(trans_proteine,trans_reads,seed,test);
	SEQAN_ASSERT_EQ(test.position_read[0],0);
	SEQAN_ASSERT_EQ(test.begin_read[0],2);
	SEQAN_ASSERT_EQ(test.end_read[0],3);
	SEQAN_ASSERT_EQ(test.position_protein[0],4);
	SEQAN_ASSERT_EQ(test.begin_protein[0],5);
	SEQAN_ASSERT_EQ(test.end_protein[0],6);
}
// TEST APPEND_TO_MATCH_FOUND ------------------------------------------------------------------------------------------
