...
int check_values(Variable & comVal,StringSet<String<Dna> > & reads){
	...
	*/
	...
	if (comVal.size_alp<6 || comVal.size_alp>20){
		cerr << "SIZE_ALPHABET MUST BE BETWEEN 6-20 " <<endl;
		return 1;
	}
...
int PARSE_ARGUMENTS(int argc,char const ** argv,Variable & comVal){
	...
	*/
	...
	addOption(parser, seqan::ArgParseOption("a", "SIZE_ALPHABET", "SIZE OF THE REDUCED AMINO ACID ALPHABET MUST BE BETWEEN 6 AND 20",
        ArgParseArgument::INTEGER, "INT"));
