...
int translate(StrSetSA & trans_reads,String<Dna> & read,String<AminoAcid> & Alphabete,int frame){
	...
	for (int triplet=0+frame;triplet<length(read) && triplet+3<=length(read);triplet+=3){
		...
		if ((frame/3)==0) aktual_triplet = infix(read,triplet,triplet+3);
			...
			cout << "reverse genmommen"<<endl;
			aktual_triplet = infix(revComplement,triplet,triplet+3);
		}
		int translate = get_translate_from_codon(aktual_triplet,Alphabete);
...
// Schleife fuer jedes Read // Uebersetzung
	for (int read=0;read<length(Reads);++read){
		...
		for(int reading_frame=0;reading_frame<6;++reading_frame){
			...
			if (translate(trans_reads,Reads[read],Alphabete,reading_frame)==1){
