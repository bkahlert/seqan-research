...
int get_translate(String<Dna> & aktual_triplet,String<AminoAcid> & Alphabete){
...
void translate_reads(StrSetSA & trans_reads,String<Dna5> & read,String<AminoAcid> & Alphabete,int frame){
	...
	for (int triplet=0+frame;triplet<length(read) && triplet+3<length(read);triplet+=3){
		...
		appendValue(integer_code,get_translate(aktual_triplet,Alphabete));
	}
	appendValue(trans_reads,integer_code);
}

// BEKOMMT EINE PROTEINSEQUENZ UND UEBERSETZT DIESE IN DAS VEREINFACHTE ALPHABET
void translate_datenbank(StrSetSA & trans_proteine,String<AminoAcid> & protein,String<AminoAcid> & Alphabete){
	String<AminoAcid>integer_code;
	for (int position=0;position<length(protein);++position){
		appendValue(integer_code,Alphabete[(int)protein[position]]);
	}
