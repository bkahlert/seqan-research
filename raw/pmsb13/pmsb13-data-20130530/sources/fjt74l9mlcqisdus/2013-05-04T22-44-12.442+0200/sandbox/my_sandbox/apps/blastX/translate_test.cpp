...
SEQAN_DEFINE_TEST(test_my_app_translate_reads)
{
	...
	for (int i=0;i<length(trans_reads);++i) cout << trans_reads[i]<<endl;
	/*
	SEQAN_ASSERT_EQ(trans_reads[0],"CQCCDC");
	SEQAN_ASSERT_EQ(trans_reads[1],"NACCA");
	SEQAN_ASSERT_EQ(trans_reads[2],"RDDCD");
