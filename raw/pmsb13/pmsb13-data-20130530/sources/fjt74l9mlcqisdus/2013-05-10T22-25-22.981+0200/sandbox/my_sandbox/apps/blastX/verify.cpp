...
int verify_seed_match(String<AminoAcid> protein,String<AminoAcid> & read,Match_found & verify_found,int & begin, int & end, int position_protein){
	TAlign align;
	resize(rows(align),2);
	...
	if (score>0){
		...
		if (intervall.i1!=-1 && intervall.i2!=-1) append_to_match_found(verify_found,intervall,position_protein)
		else return 1;
	}
	return 0;
		...
		for (int i=0;i<length(seed_found.position_read);++i){
			if (read_number == seed_found.position_read[i]){
				...
				if (begin>=0 && end<=length(trans_proteine[seed_found.position_protein[i]]) && known_position(begin,end,pair_position)){
					...
					verify_seed_match(infix(Proteine[seed_found.position_protein[i]],begin,end),real_trans_reads[read_number],verify_found,begin,end,seed_found.position_protein[i]);
				}
			}	
		}
		read_number ++;
