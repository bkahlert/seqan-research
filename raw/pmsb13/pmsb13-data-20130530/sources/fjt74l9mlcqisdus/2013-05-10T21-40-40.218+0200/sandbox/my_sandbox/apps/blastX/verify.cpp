...
Pair<unsigned int, unsigned int> get_position_in_prot(TAlign & align,int & begin,int & end){
	...
	if (row(align,1)[0]!='-') return_pos.i1 = begin;
	if (row(align,1)[length(row(align,1))-1]!='-') return_pos.i2 = end;

	
	cout << begin << "\t"<< end<<endl;
	
	cout << return_pos.i1 << "\t"<< return_pos.i2<<endl;
...
int verify_seed_match(String<AminoAcid> protein,String<AminoAcid> & read,Match_found & verify_found,int & begin, int & end){
	...
	if (score>0){
		...
		cout << align <<endl;
		Pair<unsigned int, unsigned int> position = get_position_in_prot(align,begin,end);
		...
		for (int i=0;i<length(seed_found.position_read);++i){
			if (read_number == seed_found.position_read[i]){
				...
				if (begin>=0 && end<=length(trans_proteine[seed_found.position_protein[i]]) && known_position(begin,end,pair_position)){
					...
					verify_seed_match(infix(Proteine[seed_found.position_protein[i]],begin,end),real_trans_reads[read_number],verify_found,begin,end);
				}
			}	
		}
		read_number ++;
