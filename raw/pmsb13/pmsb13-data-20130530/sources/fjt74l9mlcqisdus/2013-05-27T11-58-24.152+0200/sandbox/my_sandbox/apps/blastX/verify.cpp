...
int verify_seed_match(String<AminoAcid> & protein, String<AminoAcid> & read, Match_found & verify_found, unsigned & begin, unsigned & end, unsigned & position_protein, unsigned & position_read, int & database_length){
	...
	*@param database_length Number of sequences in the proteindatabase
	...
	*/
	...
	double e_value = 0.035*database_length*protein_length*read_length*exp(-0.252*score);
	if (e_value<treshold){	
...
int VERIFY_ALL(Match_found & seed_found, unsigned & distance,StrSetSA & trans_proteine,StrSetSA & real_trans_reads,Match_found & verify_found,StrSetSA & proteine, int & database_length){
        ...
        for (unsigned i=0;i<length(seed_found.position_read);++i){
            if (read_number == seed_found.position_read[i]){
                ...
                if (begin<=length(trans_proteine[seed_found.position_protein[i]]) && end<=length(trans_proteine[seed_found.position_protein[i]]) && known_position(begin,end,pair_position)){
					...
					verify_seed_match(protein,real_trans_reads[read_number],verify_found,begin,end,seed_found.position_protein[i],read_number,database_length);
                }
            }     
        }
        read_number ++;
