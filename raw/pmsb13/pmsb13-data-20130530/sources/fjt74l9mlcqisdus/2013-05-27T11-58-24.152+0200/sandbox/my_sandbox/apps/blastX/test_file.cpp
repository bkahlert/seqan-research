...
SEQAN_DEFINE_TEST(test_my_app_append_to_match_found)
{
	...
	Pair<unsigned int,unsigned int> z (3,0);
	
	StringSet<unsigned int> found_reads;
	for (int i=0;i<2;++i) appendValue(found_reads,2);
	append_to_match_found(test,x,y,z,found_reads);
	SEQAN_ASSERT_EQ(test.position_read[0],(unsigned)1);
	SEQAN_ASSERT_EQ(test.begin_read[0],(unsigned)1);
	SEQAN_ASSERT_EQ(test.end_read[0],(unsigned)4);
	SEQAN_ASSERT_EQ(test.position_protein[0],(unsigned)1);
	SEQAN_ASSERT_EQ(test.begin_protein[0],(unsigned)10);
	SEQAN_ASSERT_EQ(test.end_protein[0],(unsigned)13);
...
SEQAN_DEFINE_TEST(test_my_app_find_matches)
{
	...
	SEQAN_ASSERT_EQ(result,0);
	SEQAN_ASSERT_EQ(test0.position_read[0],(unsigned)0);
	SEQAN_ASSERT_EQ(test0.begin_read[0],(unsigned)0);
	SEQAN_ASSERT_EQ(test0.end_read[0],(unsigned)3);
	SEQAN_ASSERT_EQ(test0.position_protein[0],(unsigned)0);
	SEQAN_ASSERT_EQ(test0.begin_protein[0],(unsigned)4);
	SEQAN_ASSERT_EQ(test0.end_protein[0],(unsigned)7);

	clear(trans_proteine);
	clear(trans_reads);

	appendValue(trans_reads,"RCQ");
	appendValue(trans_proteine,"QCQQCQCQ");
	appendValue(trans_proteine,"QCQQCRCQ");
	Match_found test;
	result = FIND_MATCHES(trans_proteine,trans_reads,seed,distance,test);
	SEQAN_ASSERT_EQ(result,0);
	SEQAN_ASSERT_EQ(test.position_read[0],(unsigned)0);
	SEQAN_ASSERT_EQ(test.begin_read[0],(unsigned)0);
	SEQAN_ASSERT_EQ(test.end_read[0],(unsigned)3);
	SEQAN_ASSERT_EQ(test.position_protein[0],(unsigned)1);
	SEQAN_ASSERT_EQ(test.begin_protein[0],(unsigned)5);
	SEQAN_ASSERT_EQ(test.end_protein[0],(unsigned)8);

}
// TEST APPEND_TO_FIND_MATCHES -----------------------------------------------------------------------------------------


// TEST APPEND_TO_MATCH_FOUND2 -----------------------------------------------------------------------------------------------
SEQAN_DEFINE_TEST(test_my_app_append_to_match_found2)
{
	Match_found verify_found;
	Pair<int,int> intervall_protein (4,7);
	unsigned position_protein = 8;
	unsigned position_read = 9;
	int score = 109;
	int result = append_to_match_found(verify_found,intervall_protein,position_protein,position_read, score);
	SEQAN_ASSERT_EQ(verify_found.position_protein[0],(unsigned)8);
	SEQAN_ASSERT_EQ(verify_found.begin_protein[0],(unsigned)4);
	SEQAN_ASSERT_EQ(verify_found.end_protein[0],(unsigned)7);
	SEQAN_ASSERT_EQ(verify_found.position_read[0],(unsigned)9);
	SEQAN_ASSERT_EQ(verify_found.score[0],109);
	SEQAN_ASSERT_EQ(result, 0);
}
// TEST APPEND_TO_MATCH_FOUND2 -----------------------------------------------------------------------------------------------


// TEST GET_POSITION_IN_PROT -------------------------------------------------------------------------------------------------
SEQAN_DEFINE_TEST(test_my_app_get_position_in_prot)
{
	Pair<int,int> result;
	String<AminoAcid> protein = "AAAAAAAVVVVAAAAAAA";
	String<AminoAcid> read = "AAAAVVVVAAAAA";
	TAlign align;
	resize(rows(align),2);
	assignSource(row(align,0),protein);
	assignSource(row(align,1),read);
	globalAlignment(align,Score<int,ScoreMatrix<AminoAcid, Blosum80_> >(),NeedlemanWunsch());
	unsigned begin= 45;
	unsigned end= 63 ;
	result = get_position_in_prot(align,begin,end);
	SEQAN_ASSERT_EQ(result.i1,48);
	SEQAN_ASSERT_EQ(result.i2,63);
}
// TEST GET_POSITION_IN_PROT -------------------------------------------------------------------------------------------------
...
SEQAN_DEFINE_TEST(test_my_app_verify_seed_match)
{
	...
	String<AminoAcid> protein = "QQEECFDCDEEECFDDQF";
	int database_length = length(protein);
	String<AminoAcid> read = "QQEECF";
	unsigned begin = 0;
	unsigned end = 5;
	unsigned position_protein = 0;
	unsigned position_read = 0;
	Match_found verify_found;
	int result = verify_seed_match(protein, read, verify_found, begin, end, position_protein, position_read, database_length);
...
SEQAN_DEFINE_TEST(test_my_app_verify_all)
{
	...
	//Ohne reduziertes Alphabet uebersetzt	
	StrSetSA trans_reads;
	appendValue(trans_reads, "CRCC");
	appendValue(trans_reads, "NRLR");	
	appendValue(trans_reads, "RNRC");
	//Mit reduziertes Alphabet uebersetzt	
	StrSetSA real_trans_reads;
	appendValue(real_trans_reads, "SLSS");
	appendValue(real_trans_reads, "AYPL");	
	appendValue(real_trans_reads, "LILS");
	unsigned seed = 2;	
	//Erlaubte missmatches 	
	unsigned distance = 0;	
	//Richtige Positionen der verifizierten matches
	Match_found seed_found;
	append(seed_found.position_read, 1);
	append(seed_found.position_read, 2);
	append(seed_found.position_read, 2);
	append(seed_found.position_protein, 1);
	append(seed_found.position_protein, 1);
	append(seed_found.position_protein, 1);
	append(seed_found.begin_protein, 6);
	append(seed_found.begin_protein, 0);
	append(seed_found.begin_protein, 5);
	append(seed_found.end_protein, 8);
	append(seed_found.end_protein, 4);
	append(seed_found.end_protein, 9);
	//Matches suchen und Informationen in die Klasse speichern
	Match_found seed_found2;
	int result_find = FIND_MATCHES(trans_proteine,trans_reads,seed,distance,seed_found2);	
	//Matches verifizieren	
	Match_found verify_found;
	int result_verify = VERIFY_ALL(seed_found2, distance, trans_proteine, real_trans_reads, verify_found, proteine, database_length);
	//Vergleich der Ergebnisse
	for(unsigned i = 0; i < length(verify_found.position_read); ++i){SEQAN_ASSERT_EQ(verify_found.position_read[i], seed_found.position_read[i]);}
	for(unsigned i = 0; i < length(verify_found.position_protein); ++i){SEQAN_ASSERT_EQ(verify_found.position_protein[i], seed_found.position_protein[i]);}
	for(unsigned i = 0; i < length(verify_found.begin_protein); ++i){SEQAN_ASSERT_EQ(verify_found.begin_protein[i], seed_found.begin_protein[i]);}
	for(unsigned i = 0; i < length(verify_found.end_protein); ++i){SEQAN_ASSERT_EQ(verify_found.end_protein[i], seed_found.end_protein[i]);}
...
SEQAN_BEGIN_TESTSUITE(test_my_app_funcs)
{
	...
	// verify.cpp
	SEQAN_CALL_TEST(test_my_app_append_to_match_found2);
	SEQAN_CALL_TEST(test_my_app_get_position_in_prot);
