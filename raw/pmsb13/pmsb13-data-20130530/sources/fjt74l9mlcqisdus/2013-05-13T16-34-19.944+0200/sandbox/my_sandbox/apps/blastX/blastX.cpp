...
//------------------------------------------------------------------------------------------------
	...
	for(int alp=0;alp<length(alphabets);++alp){
		...
		// -> output.cpp
		write_to_file(seed_found, proteinID, readID,comVal.numb_alp);
		
		// Matches werden verifiziert und in verify_found gespeichert
		// -> verify.cpp	
		Match_found verify_found;
		if (verify_all(seed_found,comVal.hamming_distance,trans_proteine,trans_reads,verify_found,alphabets)) return 1;
		
		// Verifizierte matches werden in eine Textdatei geschrieben
		// -> output.cpp
		write_to_file(verify_found, proteinID, readID, reads,comVal.numb_alp);
