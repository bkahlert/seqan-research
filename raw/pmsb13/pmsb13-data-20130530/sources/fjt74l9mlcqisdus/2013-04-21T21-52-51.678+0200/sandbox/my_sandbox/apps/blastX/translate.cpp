...
void translate_datenbank(StringSet<String<int>> & trans_proteine,String<AminoAcid> & protein,String<int> & Alphabete){
	String<int>integer_code;
	for (int position=0;position<length(protein) && position+3<length(protein);++position){
		int pos = get_AminoAcidPos((int)protein[position])
		appendValue(integer_code,Alphabete[pos]);
	}
