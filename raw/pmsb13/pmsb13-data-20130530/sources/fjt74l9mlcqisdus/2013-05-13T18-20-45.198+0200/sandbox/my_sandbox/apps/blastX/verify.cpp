...
int verify_seed_match(String<AminoAcid> protein,String<AminoAcid> & read,Match_found & verify_found,int & begin, int & end, int position_protein,int & position_read,int distance){
	...
	*/
	...
	int score = globalAlignment(align,Score<int,ScoreMatrix<AminoAcid, Blosum80_>>(),NeedlemanWunsch());	//to-do jochen!!!!!!!!!!!!!!!!!!!!!
...
int verify_all(Match_found & seed_found, int & distance,StrSetSA & trans_proteine,StrSetSA & trans_reads,Match_found & verify_found,StringSet<String<Dna> > & reads,StrSetSA & proteine,StrSetSA & alphabets){
    ...
    */
    StrSetSA real_trans_reads = trans_reads;
    int read_number = 0;
    do{
        ...
        for (int i=0;i<length(seed_found.position_read);++i){
            if (read_number == seed_found.position_read[i]){
                ...
                if (begin>=0 && end<=length(trans_proteine[seed_found.position_protein[i]]) && known_position(begin,end,pair_position)){
                    ...
                    verify_seed_match(infix(proteine[seed_found.position_protein[i]],begin,end),real_trans_reads[read_number],verify_found,begin,end,seed_found.position_protein[i],read_number,distance);
                }
            }     
        }
        read_number ++;
