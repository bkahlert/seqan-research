...
int verify_seed_match(String<AminoAcid> & protein, String<AminoAcid> & read, Match_found & verify_found, unsigned & begin, unsigned & end, unsigned & position_protein, unsigned & position_read, unsigned & distance){
	...
	*@param distance Zugelassene missmatches
...
int VERIFY_ALL(Match_found & seed_found, unsigned & distance,StrSetSA & real_trans_proteine,StrSetSA & trans_reads,Match_found & verify_found,StrSetSA & proteine){
        ...
        for (unsigned i=0;i<length(seed_found.position_read);++i){
            if (read_number == seed_found.position_read[i]){
                ...
                if (begin<=length(trans_proteine[seed_found.position_protein[i]]) && end<=length(trans_proteine[seed_found.position_protein[i]]) && known_position(begin,end,pair_position)){
					...
					verify_seed_match(protein,real_trans_reads[read_number],verify_found,begin,end,seed_found.position_protein[i],read_number,distance);
                }
            }     
        }
        read_number ++;
