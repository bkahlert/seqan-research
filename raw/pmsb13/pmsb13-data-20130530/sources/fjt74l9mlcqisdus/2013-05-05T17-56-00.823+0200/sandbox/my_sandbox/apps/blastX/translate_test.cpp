...
SEQAN_DEFINE_TEST(test_my_app_translate_database)
{
	...
	int result = translate_database(read,trans_proteine,alphabet[0]);
	SEQAN_ASSERT_EQ(length(trans_database),2);
	
	
	SEQAN_ASSERT_EQ(trans_proteine[0],"QQNCCQCQCCQCQCC");
	SEQAN_ASSERT_E(trans_proteine[1],"CDQNRQNCDCCDRCCQQQQC");
}
// TEST TRANSLATE_DATABASE ---------------------------------------------------------------------------------------------
