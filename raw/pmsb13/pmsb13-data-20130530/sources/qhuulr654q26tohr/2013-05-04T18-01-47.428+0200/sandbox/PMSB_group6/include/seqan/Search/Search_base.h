...
int Search(String<PerformanceSample> & performance, int argc, char const ** argv) {
	...
	GFastaRecord<TScore> queryRecord;
	GFastaRecord<TScore> referenceRecord;
	GFastaRecord<TScore> clusterRecord;
	...
	while (!readRecord(queryRecord.id, queryRecord.seq, myQueries)) {
        ...
        while(!readRecord(referenceRecord.id, referenceRecord.seq, myMaster)) {
			GScore<TScore>(score, queryRecord, referenceRecord, scoringScheme);
			GScoreStorage<TScore> scoreStorage;
			GEvaluateScore<TScore>(scoreStorage, score, referenceRecord);
        }
        open(myCluster, toCString(clusteredDatabase));
        while(!readRecord(clusterRecord.id, clusterRecord.seq, myCluster)) {
			if (GCheckClusterMaster<TScore>) {
				GComputeMatch<TScore>(matches, queryRecord, clusterRecord, scoringScheme);
            }
        }
        GPostProcessMatches<TScore>(matches);
		GWriteMatches<TScore>(myOutput, matches, queryRecord)
