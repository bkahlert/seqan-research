...
int main(int argc, char const ** argv) {
	...
	if (sequenceFormat=="DNA"){
		...
		int res = GCluster< , ,8>(performance, inPath, outPath, outMasterPath, sequenceFormatExample, threshold, lengthThreshold, scoreMode);
		if (res!=0) {
	...
	if (sequenceFormat=="AMINOACID"){
		...
		int res = GCluster< , ,3>(performance, inPath, outPath, outMasterPath, sequenceFormatExample, threshold, lengthThreshold, scoreMode);
		if (res!=0) {
			cerr << "Error in function GCluster" << endl;
			return 1;
		}
