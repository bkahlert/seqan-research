...
int main(int argc, char const ** argv) {
	...
	GSearch(matchesSoll, dataBaseLength, performance_uncl, inDataBasePath, "", inQueriesPath, matchFile_uncl, 20, scoreMode);
	...
	for (int i=0; i<length(clustDatabases);++i){
		...
		GSearch(matchesIs, dataBaseLength, performance, clustDatabases[i], masterSequenceFiles[i], inQueriesPath, matchFiles[i], 20, scoreMode);
