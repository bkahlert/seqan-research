...
int main(int argc, char const ** argv) {
	...
	addDescription(parser,"Searching a query in a large database can be accelerated if the sequences are first assign to a number of clusters representing similar sequences. The query is then first matched against the cluster masters and then against every sequence in the cluster. There are three possible scoring modes: LOCAL_ALIGNMENT_MODE suming up local alignments, GLOBAL_ALIGNMENT_MODE (no gap costs) and the recommended COMMON_QGRAM_MODE where common q-grams (DNA/RNA=8;AMINOACID=3) are counted. The q-gram score is the fraction of q-grams of the query found in the target multiplied with 1000. GCluster reads a database in fasta format and creates a new clustered database as well as a file storing all master sequences.");
