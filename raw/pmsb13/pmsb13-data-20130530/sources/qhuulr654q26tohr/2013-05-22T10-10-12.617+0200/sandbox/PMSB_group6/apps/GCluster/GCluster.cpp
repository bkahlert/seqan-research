...
int main(int argc, char const ** argv) {
	...
	setShortDescription(parser, "A part of Ginger.\nClustering of a fasta database.");
	setVersion(parser, "1.0");
	setDate(parser, "May 2013");
	addDescription(parser,"Reads a database in fasta format and creates a new database in which each sequence is assigned to a similar sequence (cluster master).");
	...
	addText(parser,"Explanation for different modes");
    addOption(
        parser,
	...
	addListItem(parser,"GCluster \\fBpfam.100K.fasta ClusteredDatabase_OutPath ClusterMasters_OutPath\\fP -f AMINOACID -t 80", "dummyExample");
	addListItem(parser,"GCluster \\fBrfam.10K.fasta ClusteredDatabase_OutPath ClusterMasters_OutPath\\fP -f RNA -t 80 -l 0.1", "dummyExample");
