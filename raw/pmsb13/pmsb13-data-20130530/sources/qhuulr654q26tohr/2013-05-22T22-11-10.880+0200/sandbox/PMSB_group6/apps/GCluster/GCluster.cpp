...
int main(int argc, char const ** argv) {
	...
	addDescription(parser,"Searching a query in a large database can be accelerated if the sequences are first assign to a number of clusters representing similar sequences. The query is then first matched against the cluster masters and then against every sequence in the cluster. This tool reads a database in fasta format and creates a new clustered database as well as a file storing all master sequences.");
	...
	addListItem(parser,"GCluster \\fBpfam.100K.fasta ClusteredDatabase_OutPath ClusterMasters_OutPath\\fP -f AMINOACID -t 80", "Cluster members have 8% similar 3-grams to the cluster master.");
	addListItem(parser,"GCluster \\fBrfam.10K.fasta ClusteredDatabase_OutPath ClusterMasters_OutPath\\fP -f RNA -t 80 -l 0.1", "Cluster members have 8% similar 8-grams and their length varies only by 10% from the cluster master.");
