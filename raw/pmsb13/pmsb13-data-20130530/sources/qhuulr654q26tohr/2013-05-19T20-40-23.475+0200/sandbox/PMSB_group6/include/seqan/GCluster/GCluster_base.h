...
int readCommandlineParameters(TPath & inPath, TPath & outPath, TPath & outMasterPath,
							  string & sequenceFormat, TThres & threshold, double & lengthThreshold, string & scoreMode, int & argc, char const ** & argv) {
...
template<typename TPath, typename TSequence, typename TScore>
int GCluster(String<PerformanceSample> & performance, TPath & inPath, TPath & outPath, TPath & outMasterPath,
	...
	int res = readCommandlineParameters(inPath, outPath, outMasterPath, sequenceFormat, threshold, lengthThreshold, scoreMode, argc, argv);
    // If parsing was not successful then exit with code 1 if there were errors.
    // Otherwise, exit with code 0 (e.g. help was printed).
    if (res != seqan::ArgumentParser::PARSE_OK)
