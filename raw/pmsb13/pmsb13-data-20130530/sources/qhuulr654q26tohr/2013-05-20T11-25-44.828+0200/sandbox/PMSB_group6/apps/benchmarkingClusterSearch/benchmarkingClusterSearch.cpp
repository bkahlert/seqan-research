...
int main(int argc, char const ** argv) {
	...
	GSearch(matchesSoll, dataBaseLength, performance_uncl, inDataBasePath, "", inQueriesPath, matchFile_uncl, 20, 8, scoreMode);
	...
	for (int i=0; i<length(clustDatabases);++i){
		...
		GCluster(performance,inDataBasePath,clustDatabases[i],masterSequenceFiles[i],sequenceFormatExample,clusterThresholds[i],0.5, 8,scoreMode);
		
		GSearch(matchesIs, dataBaseLength, performance, clustDatabases[i], masterSequenceFiles[i], inQueriesPath, matchFiles[i], 20, 8, scoreMode);
