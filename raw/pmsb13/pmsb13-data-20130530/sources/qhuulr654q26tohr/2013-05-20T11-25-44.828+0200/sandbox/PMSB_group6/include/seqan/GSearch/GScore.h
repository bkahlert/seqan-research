...
int GScore(TScore &score, GFastaRecord<TSequence> &newRecord, GFastaRecord<TSequence> &clusterRecord,
		   TScoringScheme &scoringScheme, unsigned QGramSize, string &scoreMode)
{
	// LOCAL_ALIGNMENT_MODE
    if (scoreMode=="LOCAL_ALIGNMENT_MODE") {
        score=0;
        LocalAlignmentEnumerator<SimpleScore, Unbanded> enumerator(scoringScheme, 50);
...
//        ::std::cout << align << ::std::endl;
    ...
    if (scoreMode=="COMMON_QGRAM_MODE") {
        ...
        typedef Index< TSet, IndexQGram<UngappedShape<QGramSize> > > TIndex;
        TSet mySet;
