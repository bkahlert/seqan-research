...
int main(int argc, char const ** argv) {
	...
	addDescription(parser,"Searching a query in a large database can be accelerated if the sequences are first assign to a number of clusters representing similar sequences. The query is then first matched against the cluster masters and then against every sequence in the cluster. GSearch reads a fasta file containing the queries and searchs each query in a database. If a file containing master sequences is specified it checks only the sequences of the corresponding cluster assuming that the database is clustered respectively.");
