...
/**
*Comparefunction for sorting kmerCount
*highest first
*/
struct IntCmp {
...
unsigned i=0;
   ...
   // for(countIter=kmer.kmerCount.begin();countIter!=kmer.kmerCount.end(); ++countIter){
//std::cout<<countIter->first<<"\t"<<countIter->second<<std::endl;
  //  }
	/**
	*create a vector for sorting the map
	*/
    	std::vector<mypair> myvec(kmer.kmerCount.begin(), kmer.kmerCount.end());
    	assert(myvec.size() >= 6);
	...
	*/
	std::partial_sort(myvec.begin(), myvec.begin() + 6, myvec.end(), IntCmp());
    	...
    	/**
    	*write first 6 most foundet kmers
    	*/
    	for(int v=0;v<6; ++v){
		ss<<"\t"<<myvec[v].first;
    	}
    ...
    *write first 6 most foundet kmers
    */
    //for(countIter=kmer.kmerCount.begin();countIter!=kmer.kmerCount.end()&&i<6; ++countIter,++i){
      //  ss<<"\t"<<countIter->first;
//std::cout<<countIter->first<<"\t"<<countIter->second<<std::endl;
  //  }
    ss<<std::endl;
    ...
    */
	...
	unsigned j=0;
//std::cout<<"1"<<std::endl;
//std::cout<<"kmerPositionIter->first\t"<<kmerPositionIter->first<<std::endl;
    	/**
   	*write count
    	*/
//std::cout<<"2"<<std::endl;
  //      for(countIter=(kmerPositionIter->second).begin();countIter != (kmerPositionIter->second).end(); ++countIter){
//std::cout<<"kmerPositionIter->second\t"<<countIter->first<<std::endl;
//std::cout<<"kmerPositionIter->theard\t"<<countIter->second<<"\n"<<std::endl;
...
//	}
    	...
    	for(int v=0;v<6; ++v){
//std::cout<<"1"<<std::endl;
std::cout<<"wuso"<<std::endl;
std::cout<<myvec[v].first<<std::endl;

if((kmerPositionIter->second).find(myvec[v].first)!=(kmerPositionIter->second).end()){
		writeIter=(kmerPositionIter->second).find(myvec[v].first);
std::cout<<"ahhh"<<std::endl;
std::cout<<((kmerPositionIter->second).find(myvec[v].first))->first<<std::endl;
std::cout<<"ooooooooooo"<<std::endl;
std::cout<<((kmerPositionIter->second).find(myvec[v].first))->second<<std::endl;
