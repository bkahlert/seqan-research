...
SEQAN_DEFINE_TEST(test_collect_quantiles){
    ...
    seqan::String<__uint64> cum_intervals;
...
SEQAN_DEFINE_TEST(test_kmer_content){
    ...
    kc.CountKmers("ATCACGATC");

    SEQAN_ASSERT_EQ(kc.kmerPositions["AC"][0],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["AC"][3],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["AC"][5],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["ACC"][0],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["ACC"][5],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["ACG"][3],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["AT"][0],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["AT"][3],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["AT"][6],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["ATA"][3],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["ATC"][0],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["ATC"][6],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["CA"][2],2);
    SEQAN_ASSERT_EQ(kc.kmerPositions["CA"][7],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["CAC"][2],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["CAT"][2],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["CC"][1],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["CC"][6],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["CCA"][1],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["CCA"][6],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["CG"][4],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["CGA"][4],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["GA"][5],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["GAT"][5],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["TA"][4],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["TAC"][4],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["TC"][1],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["TC"][7],1);
    SEQAN_ASSERT_EQ(kc.kmerPositions["TCA"][1],1);*/

}



SEQAN_DEFINE_TEST(test_stats_grow_with_dupcheck){
    ReadStats stats(5);
    stats.dupHaystackSize = 5;
    
    SEQAN_ASSERT_MSG(stats.duplicates.empty(), "there should not by any duplicates here yet"); 

    // (1) fill the haystack
    stats.dupCheck("CCCCC");
    stats.dupCheck("CAAAA");
    stats.dupCheck("ACAAA");
    stats.dupCheck("AACAA");
   

    
    SEQAN_ASSERT_MSG(stats.duplicates.empty(), "there should not by any duplicates here yet"); 
    // (2) index should be created (including this sequence) 
    stats.dupCheck("AAACA");
    SEQAN_ASSERT_MSG(stats.duplicates.empty(), "there should not by any duplicates here yet"); 
    
    // those seq's should be searched for in the haystack created in (1)
    stats.dupCheck("AAAAC");
    SEQAN_ASSERT_MSG(stats.duplicates.empty(), "there should not by any duplicates here yet"); 
    stats.dupCheck("AAAAG");
    SEQAN_ASSERT_MSG(stats.duplicates.empty(), "there should not by any duplicates here yet"); 
    stats.dupCheck("AAAAA");
    SEQAN_ASSERT_MSG(stats.duplicates.empty(), "there should not by any duplicates here yet"); 
    
    //duplicate:
    stats.dupCheck("AACAA");
    SEQAN_ASSERT_MSG(!stats.duplicates.empty(), "there should be duplicates here"); 
    SEQAN_ASSERT_MSG(stats.duplicates.count("AACAA"), "duplicates should have this key");
    SEQAN_ASSERT_EQ(1, stats.duplicates["AACAA"]);

    //duplicate twice:
    stats.dupCheck("CAAAA"); // in haystack 1st time
    stats.dupCheck("TTAAA"); // not in haystack
    stats.dupCheck("CAAAA"); // in haystack 2nd time
    stats.dupCheck("ACAAA"); // in haystack 1st time

    SEQAN_ASSERT_MSG(!stats.duplicates.empty(), "there should be duplicates here"); 
    
    SEQAN_ASSERT_MSG(stats.duplicates.count("CAAAA"), "duplicates should have this key");
    SEQAN_ASSERT_EQ(2, stats.duplicates["CAAAA"]);
    
    SEQAN_ASSERT_MSG(stats.duplicates.count("ACAAA"), "duplicates should have this key");
    SEQAN_ASSERT_EQ(1, stats.duplicates["ACAAA"]);

}


SEQAN_DEFINE_TEST(test_output_guess_scoreinfo_sanger){

    ReadStats stats(9);
    char lowS = 33; char highS = 57; // > Sanger, Illimuna 1.8+

    stats.scoreCount[3][lowS] = 189;
    stats.scoreCount[2][highS] = 811;
   
    TSVWriter w;
    ScoreInfo score_info;

    w.guessQualScoreOffset(stats, score_info);
    SEQAN_ASSERT_EQ(score_info.offset, 33);
    SEQAN_ASSERT_EQ(score_info.minScore, lowS);
    SEQAN_ASSERT_EQ(score_info.maxScore, highS);
    SEQAN_ASSERT_EQ(score_info.description, "Sanger, Illumina 1.8+");
}
SEQAN_DEFINE_TEST(test_output_guess_scoreinfo_solexa){

    ReadStats stats(9);
    char low = 59; char high = 92; // > Solexa

    stats.scoreCount[3][low] = 189;
    stats.scoreCount[2][high] = 811;
   
    TSVWriter w;
    ScoreInfo score_info;

    w.guessQualScoreOffset(stats, score_info);
    SEQAN_ASSERT_EQ(score_info.offset, 64);
    SEQAN_ASSERT_EQ(score_info.minScore, low);
    SEQAN_ASSERT_EQ(score_info.maxScore, high);
    SEQAN_ASSERT_EQ(score_info.description, "Solexa");
}
SEQAN_DEFINE_TEST(test_output_guess_scoreinfo_illumina_old){

    ReadStats stats(9);
    char low = 64; char high = 99; // > Illumina 1.3+, Illumina 1.5+
    ...
    SEQAN_ASSERT_EQ(score_info.minScore, low);
    SEQAN_ASSERT_EQ(score_info.maxScore, high);
    SEQAN_ASSERT_EQ(score_info.description, "Solexa, Illumina 1.3+");
}
SEQAN_DEFINE_TEST(test_output_guess_scoreinfo_illumina_new){

    ReadStats stats(9);
    char low = 34; char high = 73; // > Illimuna 1.8+
...
SEQAN_BEGIN_TESTSUITE(test_seqc_stats)
{
    ...
    SEQAN_CALL_TEST(test_stats_grow_with_dupcheck);
    SEQAN_CALL_TEST(test_output_guess_scoreinfo_sanger);
    SEQAN_CALL_TEST(test_output_guess_scoreinfo_solexa);
    SEQAN_CALL_TEST(test_output_guess_scoreinfo_illumina_old);
    SEQAN_CALL_TEST(test_output_guess_scoreinfo_illumina_new);
