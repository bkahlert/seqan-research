...
void TSVWriter::writeKmerContent(KmerContent & kmer,std::stringstream & ss){
    ...
    std::map<std::string, std::map<unsigned,unsigned> >::iterator kmerPositionIter; 
    std::map<unsigned,unsigned>::iterator writeIter; 


    for(unsigned i=0,countIter=kmer.kmerCount.begin();countIter != kmer.kmerCount.begin() && i<6; ++countIter,++i){
	...
	ss<<kmerPositionIter->first;
        for(unsigned j=0,countIter=kmer.kmerCount.begin();countIter != kmer.kmerCount.begin() && j<6; ++countIter,++j){
