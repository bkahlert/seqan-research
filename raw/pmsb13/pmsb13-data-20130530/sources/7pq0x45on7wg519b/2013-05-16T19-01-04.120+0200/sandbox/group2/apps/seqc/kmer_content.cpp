...
//std::cout << "\n\nShape\n"<< seqan::value(shape)<< std::endl;
			...
			kmerPositions[seqan::value(shape)][j]++;
			//std::cout << "\n\nCount\n"<< kmerCount[kmer][j] << std::endl;
		}
...
std::cout <<"\n"<< std::endl;
 ...
 std::cout << "\nCounts\t" << std::endl;
for(std::map<std::string,std::map<int,int> >::const_iterator it = kmerPositions.begin(); it != kmerPositions.end(); ++it) 
{
for(std::map<int,int>::const_iterator itt = it->second.begin(); itt != it->second.end(); ++itt) 
    std::cout << "\nkmer:\t" << it->first <<"\npos: \t"<< itt->first << "\ncount: \t"<< itt->second <<std::endl;
