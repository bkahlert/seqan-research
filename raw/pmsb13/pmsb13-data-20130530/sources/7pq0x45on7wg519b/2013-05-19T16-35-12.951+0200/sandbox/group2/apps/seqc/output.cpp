...
*
void TSVWriter::writeKmerContent(KmerContetnt & kmer,std::stringstream & ss){
    
    ss << ">>Kmer Content\tpass" << std::endl;
    ss << "#Position\t";
    
    std::map<std::String, unsigned>::iterator countIter; 
    std::map<std::String, std::map<unsigned,unsigned> >::iterator positionIter; 
    std::map<unsigned,unsigned>::iterator writeIter; 
    typedef std::map<unsigned, seqan::String<seqan::Dna5String> > SMapType;
    SMapType sorted;
    for(countIter=kmer.kmerCount.begin();countIter != kmer.kmerCount.begin()+6; ++CountIter){
	positionIter=kmer.kmerPosition.find(countIter->first);
	for(writeIter=kmer.kmerPosition.second.begin();writeIter!=kmer.kmerPosition.second.end();++writeIter){
