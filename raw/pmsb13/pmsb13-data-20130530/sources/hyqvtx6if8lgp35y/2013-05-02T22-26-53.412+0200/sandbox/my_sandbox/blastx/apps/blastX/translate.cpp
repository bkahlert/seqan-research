...
int translate_datenbank(StrSetSA & trans_proteine,String<AminoAcid> & protein,String<AminoAcid> & Alphabete){
	String<AminoAcid>integer_code;
	for (int position=0;position<length(protein);++position){
		int Amino = (int)protein[position];
		if (Amino>=0 && Amino<=20) appendValue(integer_code,Alphabete[Amino]);
		else{
