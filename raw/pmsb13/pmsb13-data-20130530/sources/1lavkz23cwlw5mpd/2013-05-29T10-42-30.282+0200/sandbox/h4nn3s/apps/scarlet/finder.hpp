...
struct Match
{
    typedef unsigned short  TQId;
    typedef unsigned int    TSId;// many suffixes in subject-index
    typedef unsigned short  TPos;
    TQId qryId;
    TPos qryStart;
    TPos qryEnd;
...
struct BlastMatch
{
    ...
    BlastMatch(Match const & _m)
        : orig_m(_m), m(_m), ali_length(0), identities(0), positives(0), mismatches(0),
          gaps(0), gap_openings(0), eval(0.0), bitScore(0.0)
    {}

//     BlastMatch(BlastMatch const & bm2) = default;

    inline bool operator==(BlastMatch const & bm2) const
    {
         return std::tie(m, eval, bitScore)
             == std::tie(bm2.m, bm2.eval, bm2.bitScore);
    }

    inline bool operator< (BlastMatch const & bm2) const
    {
         return std::tie(m.qryId,
                         m.subjId,
                         eval,
                         bitScore)
              < std::tie(bm2.m.qryId,
