             ...
             BlastFormat<m, p, g> const & /*tag*/)
{
    ...
    start = sysTime();
    std::cout << "Realigned, extending and dumping matches..." << std::flush;
    ...
    std::vector<std::tuple<Match::TQId,   // TrueQryId
                           ...
                           > intervals;
    intervals.resize(qrysWithMatches+1);
...
//             String<BlastMatch> bmatches;
           ...
           #pragma omp critical
            {
                typename Match::TQId last_id = -1;
                for (BlastMatch const & bm : bmatches)
                {
                    int ret = 0;
                    if (last_id != bm.m.qryId)
                    {
                        last_id = bm.m.qryId;
                        ret = writeHeader(stream,
                                          qryIds[bm.m.qryId],
                                          options.dbFile,
                                          TFormat());
                        if (ret)
