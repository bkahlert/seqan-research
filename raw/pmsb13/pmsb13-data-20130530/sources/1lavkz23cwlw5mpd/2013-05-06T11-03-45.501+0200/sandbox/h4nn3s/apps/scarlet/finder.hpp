...
seedMatches2QueryMatches(MatchManager & mm, ScarletOptions const & options)
{
    ...
    clear(mm.seedRanks); // free up memory
    clear(mm.seedRefs);  // free up memory
}
...
//             else if ((m.qryId == fm.qryId) && (m.subjId == fm.subjId) )
//             {
//                 std::cout << "not joined!\nqrystarts: "
//                           << m.qryStart << '\t'
//                           << _protectUnderflow(fm.qryStart, d) << '\t'
//                           << _protectOverflow( fm.qryEnd,   d) << '\n'
//                           << "subjstarts: "
//                           << m.subjStart << '\t'
//                           << _protectUnderflow(fm.subjStart, d) << '\t'
//                           << _protectOverflow( fm.subjEnd,   d) << '\n';
//             }
        }
        // else append the current match to the "output" list
        appendValue(filteredMatches, m);
    }
    finish = sysTime() - start;
    std::cout << " done.\n" << std::flush;
    std::cout << "Runtime: " << finish << "s \n" << std::flush;
    std::cout << "No of matches before filtering: " << length(mm.matches)
              << ". Afer: " << length(filteredMatches) << "\n\n" << std::flush;

    clear(mm.matches); // free up memory
}
