             ...
             BlastFormat<m, p, g> const & /*tag*/)
{
    ...
    {
        if ((l>0) && (ms[l-1].qryId != ms[l].qryId))
            ++qrysWithMatches;
    }
    std::vector<std::tuple<Match::TQId,   // qryId
                           unsigned long, // begin of match interval
                           unsigned long> // end of match interval
                           > intervals;
    intervals.resize(qrysWithMatches, {0,0,0});
    for (unsigned long l = 0, l2 = 0; l < length(ms); ++l)
    {
        if (l == 0)
        {
            std::get<0>(intervals[l2]) = ms[l].qryId;;
            std::get<1>(intervals[l2]) = l;
        }
        // new query
        else if (std::get<0>(intervals[l2]) != ms[l].qryId)
        {
    ...
    for (unsigned long l = 0; l < length(intervals); ++l)
    {
        ...
        String<BlastMatch_> bmatches;
        for (unsigned long beg = std::get<1>(in);
             beg < std::get<2>(in);
             ++beg)
        {
            BlastMatch_ bm;
            bool good = computeBlastMatch(bm, ma, qrySeqs, subjSeqs, qryIds, subjIds,
