...
//     typedef StringSet<CharString>  TSeeds;
    ...
    //DEBUG
    unsigned maxlen = 0;
    for (auto const & q : qrySeqs)
        if (length(q) >= maxlen)
            maxlen = length(q);
    std::cout << "maxlen(query) before seeding: " << maxlen << "\n\n";
...
//     typedef typename SAValue<TSa>::Type             TSav;
    ...
    for (unsigned u = 0; u < length(seeds); ++u)
    {
        assignValueI1(value(sa,u), u);
        assignValueI2(value(sa,u), 0);
    }
    Comp<typename Value<TSa>::Type, TSeeds const> comp(seeds);
