									...
									bool firstRun)
{
	...
	if (seqan::beginPositionH(seedChain[0]) > q && seqan::beginPositionV(seedChain[0]) > q && firstRun)
	{
	...
	if (seqan::endPositionH(back(seedChain)) < seqan::length(seedChain) - q &&
		seqan::endPositionV(seqan::back(seedChain)) < seqan::length(seedChain) - q &&
		firstRun)
	{
