...
bool sortFunction (seqan::Seed<seqan::Simple> seedA, seqan::Seed<seqan::Simple> seedB)
{
	return (beginPositionH(seedA) < beginPositionH(seedB));
}
std::vector<unsigned> get_qGramArray(unsigned &maxQ, unsigned &minQ, unsigned &parts)
{
	std::vector<unsigned> qGramArray;
	if (maxQ > minQ)
	{
		qGramArray.push_back(maxQ);
	}
	if (parts > 0u && (double) (maxQ - minQ) / parts > (double) 1)
	{
		unsigned intervall = std::floor((double) (maxQ - minQ) / parts);
		unsigned temp = maxQ;

		for(unsigned i = 0; i < parts - 1; ++i)
		{
			temp -= intervall;
			qGramArray.push_back(temp);

		}
	}
	qGramArray.push_back(minQ);
	return qGramArray;
}

bool get_global_seed_chain(seqan::String<seqan::Seed<seqan::Simple> > &seedChain,
							seqan::String<seqan::Dna> &seq1, seqan::String<seqan::Dna> &seq2,
							std::pair<unsigned, unsigned> startPos,
							std::pair<unsigned, unsigned> endPos, unsigned q)
{
	if (empty(seq1) || empty(seq2) || q <= 2 ||
		endPos.first - startPos.first < q || endPos.second - startPos.second < q)
	{
		return false;
	}
	seqan::String<seqan::Dna> window1 = seqan::infix(seq1, startPos.first, endPos.first);
	seqan::String<seqan::Dna> window2 = seqan::infix(seq2, startPos.second, endPos.second);
	...
	typedef seqan::Index<seqan::String<seqan::Dna>, seqan::IndexQGram<seqan::SimpleShape> > qGramIndex;
...
#pragma omp parallel shared(set)
	...
	{
		seqan::String<seqan::Dna> qGram = seqan::infix(seq1, i, i + q);
...
bool insert_seeds_in_seedChain(seqan::String<seqan::Seed<seqan::Simple> > &seedChain,
									...
									seqan::String<seqan::Dna> &seqOne, seqan::String<seqan::Dna> &seqTwo,
									unsigned q)
{

	//check if any container is empty
	if(!startPos.empty() && !endPos.empty() &&
		!empty(seedChain) && !empty(seqOne) && !empty(seqTwo))
	{

		std::cout << "startPos[0].first = " << startPos[0].first << "  startPos[0].second = " << startPos[0].second << std::endl;
		...
		for(unsigned i = 0; i < startPos.size(); ++i)
		{
				...
				(endPos[i].second - startPos[i].second) > q )
			{
				...
				if (get_global_seed_chain(tempChain, seqOne, seqTwo, startPos[i], endPos[i], q))
				{
					#pragma omp critical
					{
					seqan::append(seedChain, tempChain);
					}
...
{
	if (seqan::empty(seedChain) || lenSeqOne < q || lenSeqTwo < q)
	{
		std::cerr << "Leere seedChain übergeben..." << std::endl;
		return false;
	}
	if (seqan::beginPositionH(seedChain[0]) > 5 * q && seqan::beginPositionV(seedChain[0]) > 5 * q)
	{
	...
	if (seqan::endPositionH(back(seedChain)) < lenSeqOne - (5 * q) &&
		seqan::endPositionV(seqan::back(seedChain)) < lenSeqTwo - (5 * q))
	{
...
bool read_FASTA(seqan::CharString path, seqan::CharString  &id, seqan::String<seqan::Dna> &seq)
{
...
bool output_alignment(seqan::CharString path, seqan::CharString &idOne, seqan::CharString &idTwo,
						seqan::Align<seqan::String<seqan::Dna>, seqan::ArrayGaps> &alignment,
						int score)
{
