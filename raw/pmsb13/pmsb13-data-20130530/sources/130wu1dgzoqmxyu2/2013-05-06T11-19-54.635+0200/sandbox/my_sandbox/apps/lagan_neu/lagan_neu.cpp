...
parseCommandLine(ModifyStringOptions & options, int argc, char const ** argv)
{
    ...
    setShortDescription(parser, "Reimplementation of (lite version) LAGAN, a global aligner");
...
int main(int argc, char const ** argv){
	...
	resize(indexShape(qgramIndex), stellarKmer);
	...
	*/
	...
	//output
	output_alignment(outputFilename, idOne, idTwo, alignment, result);
	return 0;
}
