...
parseCommandLine(LaganOptions & options, int argc, char const ** argv)
{
	...
	addOption(parser, seqan::ArgParseOption(
			"q", "qGram", "largest qGram length to use for the index.",
			seqan::ArgParseArgument::INTEGER, "UNSIGNED"));
	setMinValue(parser, "q", "4");
	setMaxValue(parser, "q", "15");
	addOption(parser, seqan::ArgParseOption("t", "threads", "number of threads to be used.",
											seqan::ArgParseArgument::INTEGER, "UNSIGNED"));
	setMinValue(parser, "t", "1");
	addOption(parser, seqan::ArgParseOption("minQ", "minimumQ", "smallest allowed qGram for index",
											seqan::ArgParseArgument::INTEGER, "UNSIGNED"));
	...
	getOptionValue(options.maxQ, parser, "q");
	getOptionValue(options.minQ, parser, "minQ");
	getOptionValue(options.t, parser, "t");
...
int main(int argc, char const ** argv){
	...
	std::cout << "q: " << options.maxQ << std::endl;
	...
	*/
	...
	unsigned maxQ = options.maxQ;
	unsigned minQ = options.minQ;
	unsigned parts = 3;
	...
	*/
		...
		//creating a container to define the gaps where to search in
		...
		std::sort(begin(seedChain), end(seedChain), sortFunction);
	}
	//debugging:
	...
	{
		if (beginPositionH(seedChain[i+1]) - endPositionH(seedChain[i]) > 100000u ||
				beginPositionV(seedChain[i+1]) - endPositionV(seedChain[i]) > 100000u)
		{
			std::cout << "Warning: large gap between two seeds detected. Memory might not be sufficent." << std::endl;
		}
	}
	...
	double elapsed_time = double(endT - startT)  / CLOCKS_PER_SEC;
