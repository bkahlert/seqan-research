...
int main(int argc, char const ** argv){
	...
	depthCounter.push_back(length(initSeedChain));

	const unsigned initQGram = 40;
	...
	while (!depthCounter.empty()){
		...
		while(q >= 12){
			...
			int startSeq1 = seqan::endPositionH(initSeedChain[0]);
			int endSeq1 = seqan::beginPositionH(initSeedChain[1]);
			int startSeq2 = seqan::endPositionV(initSeedChain[0]);
			int endSeq2 = seqan::beginPositionV(initSeedChain[1]);

			if((endSeq1 - startSeq1) > 5 * initQGram || (endSeq2 - startSeq2) > 5 * initQGram){

				seqan::String<SSeed> tempChain;

				seqan::DnaString window1 = seqan::infix(seqOne, seqan::endPositionH(initSeedChain[0]), seqan::beginPositionH(initSeedChain[1]));
				seqan::DnaString window2 = seqan::infix(seqTwo, seqan::endPositionV(initSeedChain[0]), seqan::beginPositionV(initSeedChain[1]));

				std::cout << "window1: " << window1 << std::endl;
				std::cout << "window2: " << window2 << std::endl;

				tempChain = get_global_seed_chain(window1, window2, q);


				if (depthCounter.back() > 0){
				--depthCounter.back();
				depthCounter.push_back(length(tempChain));

				seqan::insert(initSeedChain, 1, tempChain);
