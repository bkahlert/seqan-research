...
int main(int argc, char const ** argv){
	...
	*/
	...
	for (unsigned i = 0; i < length(seedChain) - 1; ++i){
		...
		unsigned endSeq1 = startPositionH(seedChain[i + 1]);
		unsigned endSeq2 = startPositionV(seedChain[i + 1]);
	...
	for(unsigned i = 0; i < startPos.size(); ++i){
		seqan::DnaString window1 = seqan::infix(seqOne, startPos[i].first, endPos[i].first);
		seqan::DnaString window2 = seqan::infix(seqTwo, startPos[i].second, endPos[i].second);
