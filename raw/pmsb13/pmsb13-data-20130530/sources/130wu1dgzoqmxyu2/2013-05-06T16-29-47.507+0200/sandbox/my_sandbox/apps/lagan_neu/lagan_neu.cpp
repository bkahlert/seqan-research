...
parseCommandLine(LaganOptions & options, int argc, char const ** argv)
{
	...
	addOption(parser, ArgParseOption("s", "compactThresh",
    ...
    getOptionValue(options.q, parser, "qGram");
    ...
    getOptionValue(options.compactThresh, parser, "s");
...
int main(int argc, char const ** argv){
	...
	*/
	...
	stellar(swiftFinder,
			...
			options.numMatches,
			false, //argument for forward/reverse search
			idOne,
			false,
			matches,
			AllLocal());
