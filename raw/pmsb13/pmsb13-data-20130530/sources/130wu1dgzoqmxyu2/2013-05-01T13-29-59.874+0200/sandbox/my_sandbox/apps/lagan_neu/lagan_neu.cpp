...
seqan::String<Seed<Simple> > get_global_seed_chain(seqan::DnaString &seq1, seqan::DnaString &seq2, unsigned q){
	...
	typedef seqan::SeedSet<ChainedSeed> SSeedSet;

	SSeedSet seedSet;

	seqan::String<Seed<Simple> > seedChain;
	seqan::String<Seed<Simple> > improvedGlobalSeedChain;
	...
	for (unsigned i = 0; i < length(seq2) - (q - 1); ++i){
		...
		while (find(myFinder, qGram)){
			...
			seqan::addSeed(seedSet, position(myFinder), i, q, seqan::Chaos());
			//position(myFinder), i, position(myFinder) + q, i + q
		}
		clear(myFinder);
	}
