...
int main(int argc, char const ** argv){
	...
	for (unsigned i = 0; i < seedsPosSeq1.size(); ++i){
		...
		cout << "Front: " << beginPositionH(front(seedSet));
		cout << endl << "back: " << beginPositionH(back(seedSet));
		cout << endl;
	}

	Iterator<SeedSet<Simple>, Standard >::Type SimpleSetIterator = begin(seedSet);

	for (; SimpleSetIterator != end(seedSet); ++SimpleSetIterator){
		std::cout << *SimpleSetIterator;
		std::cout << std::endl;
	}

	//std::cout << "test : " << length(seedSet) << std::endl;
