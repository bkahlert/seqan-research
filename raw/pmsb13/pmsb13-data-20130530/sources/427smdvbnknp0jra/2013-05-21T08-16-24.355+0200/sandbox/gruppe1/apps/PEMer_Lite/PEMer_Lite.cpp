...
int saminput(candidates &save, modifier &options, statisticals &stats)	// impotiert eine SAM-Datei und speichert sie im Format candidates
{
	...
	std::stringstream toI(SamInput.header.sequenceInfos[0].i2);
	SamInput.header.sequenceInfos[0].i2 >> g;
	std::cout << SamInput.header.sequenceInfos[0].i2 << "\t" << g << std::endl;
	/*
	...
	coverage(length(record.seq),n,(unsigned)SamInput.header.sequenceInfos[0].i2);
    return 0;
}
