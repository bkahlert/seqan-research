...
int saminput(candidates &save, modifier &options, statisticals &stats, std::vector< seqan::CharString> &header)	// impotiert eine SAM-Datei und speichert sie im Format candidates
{	
	candidate tmp;
	medianTree median;
	...
	for (unsigned i = 0; i < length(SamInput.header.sequenceInfos); ++i)
		header.push_back(SamInput.header.sequenceInfos[i].i1 + "\t" + SamInput.header.sequenceInfos[i].i2 + "\n");
