...
struct candidate
{
	...
	seqan::Pair<seqan::CharString,int,void> ref;	// speichert die Referenz
	unsigned len;	// speichert die Laenge
	form type;	// speichert den Type

	void addvalues(seqan::Pair<seqan::CharString,int,void> r,unsigned p,unsigned l,form t)
	{
		ref=r;
		pos=p;
		len=l;
...
struct modifier
{
    unsigned x,y,sd,e,s,i,d,c;
    bool p,isC;
    seqan::CharString inputFile;
    modifier() :
        x(1), y(1),e(0),sd(0),c(0),p(false),s(false),isC(false)
    {}
};
