...
int output(std::vector<std::vector<unsigned> > &input, seqan::String<seqan::CharString> &header, candidates &ref, char *out)
{
	...
	{
		outFile << "ID\tAVEERAGE_POS\tAVEERAGE_LEN\tBegin\tEND\tTYPE " << std::endl;
		if(!input.empty())
		{
			for (unsigned i=0;i<input.size();i++)
			{
				...
				if(n!=0)
				{
					...
					outFile << header[ref[*input[i].begin()].ref] << "\t" << (maxBegin+minEnd)/2<< "\t" << targetSize << "\t" << minEnd << "\t" << maxBegin << "\t" << (candidate::form)ref[*input[i].begin()].type << std::endl;
				}
			}
		}
	}
