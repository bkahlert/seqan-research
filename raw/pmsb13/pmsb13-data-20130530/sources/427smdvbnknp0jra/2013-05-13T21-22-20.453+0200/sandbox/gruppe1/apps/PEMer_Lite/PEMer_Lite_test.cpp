...
SEQAN_DEFINE_TEST(test_my_app_PEMer_Lite_devide)
{
	...
	modifier opt;
	int testdata[]={10,10,10,10,3,9,11,13,1000,200};
	for(unsigned i=1;i<=10;i++)
	{
		test.len.push_back(testdata[i]);
	...
	SEQAN_ASSERT_EQ(result.type[1],1);
	SEQAN_ASSERT_EQ(result.type[1],2);
	SEQAN_ASSERT_EQ(result.type[1],2);
	SEQAN_ASSERT_EQ(result.type[1],2);
	SEQAN_ASSERT_EQ(result.ref[1],5);
	SEQAN_ASSERT_EQ(result.ref[1],6);
	SEQAN_ASSERT_EQ(result.ref[1],8);
	SEQAN_ASSERT_EQ(result.ref[1],9);
	SEQAN_ASSERT_EQ(result.ref[1],10);
}
