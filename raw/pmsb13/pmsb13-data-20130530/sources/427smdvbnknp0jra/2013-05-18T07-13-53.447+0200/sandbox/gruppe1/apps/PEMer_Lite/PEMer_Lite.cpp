	...
	{	
		...
		if(!input.empty())
		{
			for(unsigned j=0;j<input.size();j++)
			{
				...
				{
					unsigned av=0;
					unsigned max=ref[0].pos;
					unsigned min=ref[0].pos+ref[0].len;
					unsigned n=input[j].size();
					outFile << "Cluster: " << std::endl;

					for(auto k=input[j].begin();k != input[j].end();k++)
					{
						av += (ref[*k].pos+ref[*k].len)/2;
						if(ref[*k].pos < min){min=ref[*k].pos;}
						if((ref[*k].pos+ref[*k].len) > max){max=(ref[*k].pos+ref[*k].len);}
					}
					av /= n;
					for(auto l=input[j].begin();l != input[j].end();l++)
					{
						outFile << ref[*l].ref << "\t" << av << "\t" << ref[*l].len << "\t" << min << "\t" << max << "\t" << (candidate::form)ref[*l].type << std::endl;
					}
