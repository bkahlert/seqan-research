...
unsigned coverage(unsigned l,unsigned n, unsigned g)
{
	return(l*n/g);
	/*
...
int saminput(candidates &save, modifier &options, statisticals &stats)	// impotiert eine SAM-Datei und speichert sie im Format candidates
{
	...
	std::cout << SamInput.header.records.data_capacity << std::endl;
	std::cout << SamInput.header.sequenceInfos[0].i1 << std::endl;
	std::cout << SamInput.header.sequenceInfos[0].i2 << std::endl;
	/*
	...
	std::cout << coverage(length(record.seq),n,(unsigned)SamInput.header.sequenceInfos[0].i2 << std::endl) << std::endl;
    return 0;
}
