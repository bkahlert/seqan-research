...
struct medianTree	// Struktur welche den Median Speichert, abhängig von allen ihm eingegebenen Werten mit konstantem Speicher.
{
private:
...
struct candidate
{
	...
	void addvalues(unsigned r,unsigned p,unsigned l,form t)	// mit add(r,p,l,t) wird ein CNDIDte mit allen Elementen gfüllt.
	{
		ref=r;
		pos=p;
		len=l;
...
struct modifier	// Struktur die alle Argumente aus der Kommandoeingabe speichert.
{
	...
	x ist das vielfache der Standardabweichung für Insertions.
	y ist das vielfache der Standardabweichung für Deletions.
	sd ist die Variable für die Standardabweichung.
	e ist die Variable für den Erwartungswert.
	Die boolschen Werte speichern ob die Coverage gesetzt ist und ob sortiert werden soll.
		*/
    unsigned sd,e,c;	
	double x,y;
	bool isC,s;
    ...
    modifier() :
        x(2), y(2),e(0),sd(0),c(0),p(false),s(false),isC(false)
    {}
};
...
{
	/*
	statisticals speichert für die Statistik wichtige Werte ab. Die Variablen sind selbst erklärend.
	*/
	unsigned e_value;
	unsigned standardDegression;
	double fold_sequence_coverage;
	double coverage;
	statisticals() :
...
int output(std::vector<std::vector<unsigned> > &input, seqan::String<seqan::CharString> &header, candidates &ref, modifier &options, statisticals &stats);
