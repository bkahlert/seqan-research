...
{	
	...
	while (!atEnd(SamInput))
	{
		...
		if(record.tLen>0)
		{
			...
			std::cout << record.tLen << "\t" << median.getMedian() << std::endl;
...
{
	std::cout << "devide\t" << stats.e_value << "\t" << stats.standardDegression << std::endl;
	for (unsigned i=0;i<input.size();i++)
	{
		std::cout << input[i].len << "	"<< bool first = (input[i].len>(stats.e_value+(options.y * stats.standardDegression))) << (input[i].len<(stats.e_value-(options.x * stats.standardDegression))) << std::endl;
