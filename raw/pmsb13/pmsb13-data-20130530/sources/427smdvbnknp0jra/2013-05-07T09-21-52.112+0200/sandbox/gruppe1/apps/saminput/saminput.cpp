...
void findCluster(candidate &input, std::vector<std::vector<unsigned> > &dest, candidate::form indel)
{
    ...
    dest.push_back(currentCluster);
	std::cout << "cluster_dest:" << std::endl;
	for (unsigned i=0;i<dest.size();i++)
	{
		std::copy(dest[i].begin(), dest[i].end(), std::ostream_iterator<unsigned>(std::cout, " "));
	}
}

int tsv(std::vector<std::vector<unsigned> > input)
 {	
	std::fstream outFile("/Informatik/Development/out.tsv", std::ios::out);
	...
	else
	{
		...
		{		
			std::vector<unsigned> tmp = input[i];
			for (unsigned i=0;i<tmp.size();i++)
			{
				std::copy(tmp.begin(), tmp.end(), std::ostream_iterator<unsigned>(std::cout, " "));
			}
			std::copy(tmp.begin(), tmp.end(), std::ostream_iterator<unsigned>(outFile, " "));
		}
	}
