...
int tsv(std::vector<std::vector<unsigned>> &input,char *out)
 {	
...
int main(int argc, char *argv[])
{
	...
	if(unsigned n=result.length()!=0){
		...
		for (unsigned i=0;i<n;i++)
		{
			std::cout << result.type[i] << "	" << result.pos[i] << "	" << result.length() << std::endl;
		}
	}
	...
	tsv(cluster,"/Informatik/Development/out.tsv");
	...
	tsv(cluster,"/Informatik/Development/out2.tsv");
	if(unsigned n=cluster.size()!=0){
