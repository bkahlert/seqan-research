...
int saminput(candidates &save, modifier &options, statisticals &stats, std::vector<std::string> &header)	// impotiert eine SAM-Datei und speichert sie im Format candidates
{	
	candidate tmp;
	medianTree median;
	...
	for (unsigned i = 0; i < length(SamInput.header.sequenceInfos); ++i)
		header.push_back(SamInput.header.sequenceInfos[i].i1);
		header.push_back("\t");
		header.push_back(SamInput.header.sequenceInfos[i].i2);
		header.push_back("\n");
		...
		if(!input.empty())
		{
			for(unsigned j=0;j<input.size();j++)
			{
				if(!input[j].empty())
				{
					...
					unsigned max=0;
					unsigned min=0;
					for(unsigned k=0;k<(unsigned n=input[j].size());k++)
					{
						av += ref[k].pos;
						if(ref[k].pos > max){max=ref[k].pos;}
						if(ref[k].pos < min){min=ref[k].pos;}
					}
					av /= n;
					for(unsigned k=0;k<(unsigned n=input[j].size());k++)
					{
						outFile << ref[*j].ref << "\t" << av << "\t" << ref[*j].len << "\t" << min << "\t" << max << "\t" << (candidate::form)ref[*j].type << std::endl;
					}
