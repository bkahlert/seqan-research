		...
		if(!input.empty())
		{
			...
			{	
				...
				if(n!=0)
				{
					...
					for(auto j=input[i].begin();j != input[i].end();j++)
					{
						outFile << ref[*j].ref << "\t" << ref[*j].pos << "\t" << ref[*j].len << "\t" << (maxBegin+minEnd)/2 << "\t" << targetSize << "\t" << maxBegin << "\t" << minEnd << "\t" << (candidate::form)ref[0].type << std::endl;
									
					}
					//outFile << ref[].ref << "\t" << (maxBegin+minEnd)/2<< "\t" << targetSize << "\t" << maxBegin << "\t" << minEnd << "\t" << (candidate::form)ref[0].type << std::endl;
				}
			}
		}
	}
