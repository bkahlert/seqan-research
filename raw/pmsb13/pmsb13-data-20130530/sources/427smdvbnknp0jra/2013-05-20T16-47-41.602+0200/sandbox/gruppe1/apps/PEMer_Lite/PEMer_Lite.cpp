	...
	else
	{
		...
		if(!input.empty())
		{
			for (unsigned i=0;i<input.size();i++)
			{
				...
				{
					targetSize /= n;
					ffor(std::vector<unsigned>::iterator k=input[i].begin();k != input[i].end();j++)
					{
						outFile << ref[0].ref << "\t" << (maxBegin+minEnd)/2<< "\t" << targetSize << "\t" << maxBegin << "\t" << minEnd << "\t" << (candidate::form)ref[*k].type << std::endl;
					}
