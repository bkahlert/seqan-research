	...
	else
	{
		...
		if(!input.empty())
		{
			for (unsigned i=0;i<input.size();i++)
			{
				...
				{
					targetSize /= n;
					ffor(std::vector<unsigned>::iterator j=input[i].begin();j != input[i].end();j++)
					{
						outFile << ref[0].ref << "\t" << (maxBegin+minEnd)/2<< "\t" << targetSize << "\t" << maxBegin << "\t" << minEnd << "\t" << (candidate::form)ref[*j].type << std::endl;
					}
