...
seqan::ArgumentParser::ParseResult commands(modifier &options, int argc, char const ** argv)
{
	...
	addOption(parser,seqan::ArgParseOption("d","max_del_size", "This value define the maximum size of a deletion.",seqan::ArgParseArgument::INTEGER, "INT"));
	setMinValue(parser, "max_del_size", "1");
	setDefaultValue(parser,"max_del_size","1000000");
	
	addOption(parser,seqan::ArgParseOption("i","max_insertion_size", "This value define the maximum size of a insertion.",seqan::ArgParseArgument::INTEGER, "INT"));
	setMinValue(parser, "max_insertion_size", "1");
	setDefaultValue(parser,"max_insertion_size","100000");
	
	addOption(parser,seqan::ArgParseOption("sd","standard_degression", "This value define the standard degression for length of fragments.",seqan::ArgParseArgument::INTEGER, "INT"));
	setMinValue(parser, "standard_degression", "1");

	addOption(parser,seqan::ArgParseOption("e","expected_value", "This value define the expected value for length of fragments.",seqan::ArgParseArgument::INTEGER, "INT"));
	setMinValue(parser, "expected_value", "1");

	addOption(parser,seqan::ArgParseOption("p","Print","Select to print temporary solutions."));

    addOption(parser,seqan::ArgParseOption("s","Sort_Position","Select to sort input file."));
	...
	getOptionValue(options.d, parser, "max_del_size");
		...
		if(!input.empty())
		{
			for (unsigned i=0;i<input.size();i++)
			{
				...
				if(n!=0)
				{
					...
					outFile << ref[*input[i].begin()].ref << "\t" << (maxBegin+minEnd)/2<< "\t" << targetSize << "\t" << maxBegin << "\t" << minEnd << "\t" << (candidate::form)ref[*input[i].begin()].type << std::endl;
				}
			}
		}
	}
...
int findCluster(candidates &input, std::vector<std::vector<unsigned> > &dest, candidate::form indel, modifier &options)
{
	std::vector<unsigned> currentCluster;
    unsigned currentClusterEndPos;
	unsigned window_size=427;

	//if(indel==candidate::deletion)window_size=options.d;
	//else(indel==candidate::insertion)window_size=options.i;
	...
	for(unsigned i = 0; i < input.size(); ++i)
    {
        ...
        if(input[i].type == indel)
        {
            if(currentCluster.empty())
            {
                ...
                currentClusterEndPos = input[i].pos + window_size - 1;
            }
