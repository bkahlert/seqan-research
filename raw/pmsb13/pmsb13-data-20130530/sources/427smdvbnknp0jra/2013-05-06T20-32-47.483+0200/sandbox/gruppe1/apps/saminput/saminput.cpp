...
int tsv(std::vector<std::vector<unsigned> > cluster)
 {	
	std::fstream outFile("/Informatik/Development/outter.tsv", std::ios::out);
		...
		for (unsigned i=0;i<n;i++)
			{
				...
				std::copy(cluster[i].begin(), cluster[i].end(), std::ostream_iterator<unsigned>(outFile, " "));
			}
		}
	}
