...
{
	std::nth_element( begin(input.len), begin(input.len) + input.length(),end(input.len));

	input.av=*(begin(input.len) + input.length()/2 );

	std::cout<<"median: "<<input.av<<std::endl;
	return 0;
}
...
int tsv(std::vector<std::vector<unsigned>> &input,candidate &ref, char *out)
 {	
	...
	else
	{
		...
		{	
			outFile << "Cluster " << i+1 << ":\n";
			std::copy(input[i].begin(), input[i].end(), std::ostream_iterator<unsigned>(outFile, " "));
			outFile << "\n";
		}
	}
...
{
	//argv[1] = "/Informatik/Development/ecoli_indels_and_snps_reads.sam";
	argv[1] = "/Informatik/Development/example.sam";

	candidate result,save;
	std::cout << result.av << std::endl;
	...
	tsv(cluster,result,"/Informatik/Development/out.tsv");
	/*
