...
{
	return (unsigned)(1- exp((double)(l*n/g)));
	/*
...
int saminput(candidates &save, modifier &options, statisticals &stats)	// impotiert eine SAM-Datei und speichert sie im Format candidates
{
	...
	std::cout << n << length(record.seq) << std::endl;
        ...
        if(input[i].type == indel)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if((double)overlapLen / std::min(currentClusterEndPos - input[currentCluster.back()].pos + 1, input[i].pos + input[i].len - 1) > stats.coverage){
                        currentCluster.push_back(i);
					}
