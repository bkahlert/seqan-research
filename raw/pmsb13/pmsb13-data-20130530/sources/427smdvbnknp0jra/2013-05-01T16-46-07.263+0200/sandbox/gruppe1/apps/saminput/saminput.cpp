...
struct candidate
{
	seqan::String<seqan::String<unsigned>> pos;
	seqan::String<seqan::String<unsigned>> ref;
	seqan::String<seqan::String<unsigned>> len;
};
...
template <typename I>
int analyse(seqan::String<seqan::String<char>> input,I &result)
{
...
int saminput(candidate &save, char *file)
{
	...
	{	
		...
		appendValue(save.len,length(record.seq));
		appendValue(save.ref,record.rId);
		appendValue(save.pos,record.pos);
	}
...
int devide(seqan::String<seqan::String<char>> input,candidate result)
{
	unsigned tmp[2];
	analyse(input,tmp);
