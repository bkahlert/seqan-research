...
{	
	...
	for (unsigned i = 0; i < length(SamInput.header.sequenceInfos); ++i)
		header.push_back(toCString(SamInput.header.sequenceInfos[i].i1) + "\t" + toCString(SamInput.header.sequenceInfos[i].i2) + "\n");
		...
		if(!input.empty())
		{
			...
			{	
				if(!input[i].empty())
				{
					...
					{
						outFile << "	" << ref[*j].ref << "	" << ref[*j].pos << "	" << ref[*j].len << "	" << (candidate::form)ref[*j].type << std::endl;
					}
				}
				else
				{
