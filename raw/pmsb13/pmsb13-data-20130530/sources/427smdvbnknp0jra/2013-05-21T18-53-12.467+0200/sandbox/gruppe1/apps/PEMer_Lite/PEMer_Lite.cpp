...
int coverage(modifier &options, statisticals &stats, unsigned l,unsigned n, unsigned g)
{
	if(options.isC)
	{
		stats.coverage=1/std::exp((double)options.c);
	}else
	{
		stats.coverage=1/std::exp((double)(l*n/g));
	}
...
int output(std::vector<std::vector<unsigned> > &input, seqan::String<seqan::CharString> &header, candidates &ref, char *out)
{
	...
	{
		outFile << "ID\tAVEERAGE_POS\tAVEERAGE_LEN\tBegin\tEND\tTYPE{tCLUSTER_SIZE " << std::endl;
		if(!input.empty())
		{
			for (unsigned i=0;i<input.size();i++)
			{
				...
				if(!input[i].empty())
				{
					for(std::vector<unsigned>::iterator j=input[i].begin();j != input[i].end();j++)
					{
						...
						if((ref[*j].pos) < minEnd||minEnd==0){minEnd=(ref[*j].pos);}
					}
				}
				if(n!=0)
				{
					...
					outFile << header[ref[*input[i].begin()].ref] << "\t" << (maxBegin+minEnd)/2<< "\t" << targetSize << "\t" << minEnd << "\t" << maxBegin << "\t" << (candidate::form)ref[*input[i].begin()].type << "\t" << n << std::endl;
				}
			}
		}
	}
...
void findCluster(candidates &input, std::vector<std::vector<unsigned> > &dest, candidate::form indel, statisticals &stats)
{
    ...
    bool emptyFlag = true;
	double coverage=stats.coverage;
        ...
        if(input[i].type == indel)
        {
            ...
            else
            {
                ...
                else
                {
					...
					if((double)overlapLen / std::min(currentClusterEndPos - input[currentCluster.back()].pos + 1, input[i].pos + input[i].len - 1) >  1 - coverage*stats.e_value){
                        currentCluster.push_back(i);
					}
