			...
			{				
				if(!input[i].empty())
				{
					...
					if(n!=0)
					{
						...
						outFile << header[ref[*input[i].begin()].ref] << "\t" << (maxBegin+minEnd)/2<< "\t" << targetSize << "\t" << ref[*input[i].begin()].pos << "\t" << ref[*input[i].end()].pos << "\t" << (candidate::form)ref[*input[i].begin()].type << std::endl;
					}
				}
			}
		}
