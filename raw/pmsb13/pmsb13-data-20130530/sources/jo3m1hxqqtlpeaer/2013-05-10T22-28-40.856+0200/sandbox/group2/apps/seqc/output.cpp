 ...
 */
void TSVWriter::guessQualScoreOffset(ReadStats & stats){
    ...
    bool hasSangerScore = false;
    std::cout << seqan::length(stats.scoreCount[0]);
    for(char score = 58; score >= 0 && !hasSangerScore; --score){
        std::cout << " score: " << (unsigned)score << std::endl;
        for(unsigned pos = stats.getSize()-1; pos > 0 && !hasSangerScore; --pos){
           
            std::cout << "pos: " << pos << " score: " << (unsigned)score << std::endl;

            if(stats.scoreCount[score][pos]>0){
                hasSangerScore = true;
                std::cout << "found Sanger Score in pos:" << pos << " score: " << (unsigned)score <<  << std::endl; 
            }
        }
    }
