...
void TSVWriter::writeJobSummary(ReadStats & stats, std::stringstream & ss){
        ...
        int offset;
        guessQualScoreOffset(stats, minScore, maxScore, offset, description);
        ss << "Score Range\t " << description << " [" << minScore << ":" << maxScore "]" << std::endl ;
 ...
 * result 
 */
void TSVWriter::guessQualScoreOffset(ReadStats & stats, char minScore, char maxScore, int offset, std::string & description){
