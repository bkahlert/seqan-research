...
SEQAN_DEFINE_TEST(test_output_pos_score){
    ...
    stats.collectReadStats(id, "AAAAA", "aabca");
    stats.collectReadStats(id, "AAAAA", "abcaa");
    stats.collectReadStats(id, "AAAAA", "acaba");

    TSVWriter w;
    w.writePerPosQual(ReadStats & stats, std::stringstream & ss);
