...
void TSVWriter::guessQualScoreOffset(ReadStats & stats, ScoreInfo & score_info){
    ...
    for(char s = 0; s < 255; ++s){
        ...
        for(unsigned pos = 0; pos < stats.getSize(); ++pos){
            if(stats.scoreCount[pos][(int)s]>0){
                if(s < score_info.minScore){
                    std::cout << "was min: " << (int)score_info.minScore << "newly read: " << (int)s << std::endl; 
                    score_info.minScore = s;
                }
                if(s > score_info.maxScore){
                    std::cout << "was max: " << (int)score_info.maxScore << "newly read: " << (int)s << std::endl; 
