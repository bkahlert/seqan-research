...
static const char * _QSCORE_L = "Illumina 1.8+";
static const char * _QSCORE_X = "Solexa";
static const char * _QSCORE_I = "Illumina 1.3+";
...
void TSVWriter::writePerReadQualContent(ReadStats & stats, std::stringstream & ss){
    ss <<    ">>Per sequence quality scores\tpass" << std::endl;
    ss <<  "#Quality\tCount" << std::endl;
    
    

    for(int i=stats.scoreOffset; i<seqan::length(stats.perReadQualCount); ++i){
        ss << (i-stats.scoreOffset) << "\t" << stats.perReadQualCount[i] << std::endl;
    }
                                ...
                                std::stringstream & n_content, std::stringstream & nuc_dist )
{
    ...
    for(unsigned pos = 0; pos < length(stats.nucCount); ++pos)
    {
        ...
        {
            gc_content << (pos+1) << "\t"; 
            gc_content << 100*(float)(sum_g+sum_c)/(sum_a+sum_t+1)  << std::endl;
...
void TSVWriter::writeJobSummary(ReadStats & stats, std::stringstream & ss){
        ...
        ss << "GC Content\t" << 100 * (double)sum_gc/(sum_gc + sum_at + 1) << "%"  << std::endl;
        ss << "N Content\t" << 100 * (double)sum_n/(sum_gc + sum_at + sum_n + 1) << "%"  << std::endl;
...
void TSVWriter::guessQualScoreOffset(ReadStats & stats, ScoreInfo & score_info){
    ...
    for(unsigned s = 0; s < 255; ++s){
        for(unsigned pos = 0; pos < stats.getSize(); ++pos){
            if(stats.scoreCount[pos][s]>0){
                if(s < (unsigned)score_info.minScore){
                    score_info.minScore = s;
                }
                if(s > (unsigned)score_info.maxScore){
    ...
    // determining offset
    if((unsigned)score_info.minScore < 59){
