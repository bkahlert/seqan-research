...
void ReadStats::collectQuantiles(seqan::String<unsigned> & cum_intervals, seqan::String<char> & quantiles, unsigned points)
{
    ...
    for(unsigned i = 0; i<points; ++i){
        quantities[i] = ((float)i/points)*total_sum;
        std::cout << quantities[i] << " " << ((float)i/points)*total_sum << std::endl;
    }
    //rest is is in the last quantile
    appendValue(quantities,total_sum+1);

    //search for each quantity in cum_intervals: 
    //choose the first bucket in cum_intervals that has a larger count than 
    quant_find = 0;
    for(unsigned s = 0; s<length(cum_intervals);++s){
        std::cout << "searching for " << quantities[quant_find] << " in quantile no " << s << ": " << quantiles[i] << std::endl;
        if(quantities[quant_find] <= quantiles[s]){
            std::cout << " found quantities [" << quant_find << ":" << quantities[quant_find] 
                << "] in cum intervals bucket " << s << " [" << quantiles[s] << "]";
            ++quant_find;
