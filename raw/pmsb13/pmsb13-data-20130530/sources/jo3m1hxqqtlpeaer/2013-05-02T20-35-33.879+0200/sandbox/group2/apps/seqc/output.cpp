    ...
    ss << "Base	Mean	Median	Lower Quartile	Upper Quartile	10th Percentile	90th Percentile" 
        << std::endl;
            ...
            for(unsigned pos = 0; pos < length(stats.scoreCount); ++pos){
                ...
                //this will only work if we have more than 20 reads
                unsigned steps = total_scores/20;

                unsigned interval_pos = 0;
                for (unsigned scores_counted=0; scores_counted <= total_scores; scores_counted+=steps){
//                    if(interval[interval_pos])
                }
