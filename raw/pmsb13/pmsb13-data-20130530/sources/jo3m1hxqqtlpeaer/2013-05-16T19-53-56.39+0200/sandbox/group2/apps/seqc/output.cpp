...
void TSVWriter::guessQualScoreOffset(ReadStats & stats, ScoreInfo & score_info){
    ...
    for(unsigned s = 0; s < 255; ++s){
        std::cout << "checking in score " << s << std::endl;
    ...
    */
    ...
    if(score_info.minScore < 59){
        // can only be Sanger
        score_info.description = "Sanger Scoring System";
        score_info.offset = 33;
    } else {
        //whatever it actually is, the offset of 64 is plausible, 
        //could also be a good or cleansed Sanger run 
        score_info.offset = 64;
        if(score_info.minScore < 59){
            //Solexa is the only system with negative scores from a 64 offset
            //but this can also be a good run with 33 offset
