...
void TSVWriter::guessQualScoreOffset(ReadStats & stats, ScoreInfo & score_info){
    ...
    for(int i=0; i<(sizeof(intervals)/sizeof(*intervals));++i){
        std::cout << (char)intervals[i][0] << " [" << intervals[i][1] << ":" << intervals[i][2]
             << "] contains score range [" << (int)score_info.minScore << ":" << (int)score_info.maxScore 
             << "] ?" << std::endl; 
        if(intervals[i][1] >= score_info.minScore){
