...
SEQAN_DEFINE_TEST(test_stats_grow_with_dupcheck){
    ReadStats stats(5);
    seqan::StringSet<seqan::Dna5String> dupHaystack;
    seqan::Index<seqan::StringSet<seqan::Dna5String >, seqan::IndexEsa<> > dupIndex;
    //SEQAN_ASSERT_EQ(seqan::countSequences(dupIndex),0);

    seqan::appendValue(seqan::indexText(dupIndex), "AAAGA");
    seqan::indexCreate(dupIndex,seqan::EsaSA());
    
    SEQAN_ASSERT_EQ(seqan::countSequences(dupIndex),1);
    seqan::appendValue(seqan::indexText(dupIndex), "GAAGA");
    seqan::appendValue(seqan::indexText(dupIndex), "AGAGA");
    seqan::appendValue(seqan::indexText(dupIndex), "AAGGA");
    seqan::appendValue(seqan::indexText(dupIndex), "AAAGG");
    SEQAN_ASSERT_EQ(seqan::countSequences(dupIndex),5);
   
    //Add an already existing sequence
    seqan::appendValue(seqan::indexText(dupIndex), "AAAGA");
    SEQAN_ASSERT_EQ(seqan::countSequences(dupIndex),6);

    seqan::indexCreate(dupIndex,seqan::EsaSA());
    //seqan::resize(indexText(dupIndex), 6);
    
    //dupIndex(dupHaystack);    
    
    seqan::Finder<seqan::Index<seqan::StringSet<seqan::Dna5String>, seqan::IndexEsa<> > > dupFinder(dupIndex);
    seqan::find(dupFinder, "AAGGA");
    SEQAN_ASSERT_EQ(3,seqan::getSeqNo(seqan::position(dupFinder)));
    
    seqan::clear(dupFinder);
    seqan::find(dupFinder, "TTTTT");
    SEQAN_ASSERT_EQ(false,seqan::getSeqNo(seqan::position(dupFinder)));
    SEQAN_ASSERT_EQ(6,seqan::countSequences(dupIndex));
    
    seqan::clear(dupFinder);
    seqan::find(dupFinder, "AAAGA");
    SEQAN_ASSERT_EQ(0,seqan::getSeqNo(seqan::position(dupFinder)));



    //SEQAN_ASSERT_EQ(0,seqan::position(seqan::find(dupFinder, "TTTTT")));
    //SEQAN_ASSERT_EQ(0,seqan::position(seqan::find(dupFinder, "AAAGA")));
    ...
    stats.dupCheck("AAAAT");
    SEQAN_ASSERT_EQ("foo", "bar");
