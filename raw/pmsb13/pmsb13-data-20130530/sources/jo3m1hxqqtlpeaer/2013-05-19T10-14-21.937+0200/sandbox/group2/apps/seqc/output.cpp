...
void TSVWriter::writeSeqDuplSeq(ReadStats & stats,std::stringstream & ss){
    ...
    for(sit=sorted.rbegin();sit!=sorted.rend() && (maxDuplSeqPrint > 0);++sit){
        for(unsigned i = 0; i < length(sit->second); ++i){
            ss << maxDuplSeqPrint << "\t" << sit->first << "\t" << sit->second[i] << std::endl; 
            --maxDuplSeqPrint;
        }
    }
