...
void TSVWriter::writePerPosQual(ReadStats & stats, std::stringstream & ss)
{
    ...
    ss << "Base	Mean	Median	Lower Quartile	Upper Quartile	10th Percentile	90th Percentile" 
        ...
        for(unsigned pos = 0; pos < length(stats.scoreCount[0]); ++pos){
            for(unsigned score = 0; score < length(stats.scoreCount); ++score){
                ss << "In Pos " << pos << " score " << score << " counted: " << stats.scoreCount[score][pos] << std::endl;
            }
            ss << std::endl;
        }
