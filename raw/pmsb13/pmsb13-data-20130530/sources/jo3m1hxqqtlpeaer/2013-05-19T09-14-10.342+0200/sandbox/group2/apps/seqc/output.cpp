...
typedef std::map<seqan::Dna5String, unsigned> DMap;

struct get_second : public std::unary_function<DMap::value_type, unsigned>
{
    unsigned operator()(const DMap::value_type& value) const
    {
        return value.second;
    }
};
...
void TSVWriter::writeSeqDuplSeq(ReadStats & stats,std::stringstream & ss){
    ...
    std::vector<unsigned> my_vals;
    std::transform(stats.duplicates.begin(), stats.duplicates.end(), 
        std::back_inserter(my_vals), get_second() );
}
