...
int TSVWriter::writeAll(ReadStats & stats, std::string const outdir){
    ...
    writeNucContent(stats, gc_content, n_content);    
        ...
        std:stringstream & n_content)
{
    ...
    for(unsigned pos = 0; pos < length(stats.nucCount); ++pos)
    {
        ...
        sum_cg += sum(stats.nucCount[pos][(seqan::Dna5)'C']);
        sum_cg += sum(stats.nucCount[pos][(seqan::Dna5)'G']);
        
        sum_all += sum_cg;
        sum_all += sum(stats.nucCount[pos][(seqan::Dna5)'A']);
        sum_all += sum(stats.nucCount[pos][(seqan::Dna5)'T']);
        ...
        {
            gc_content << (pos+1) << "\t"; 
            gc_content << (float)sum_cg/sum_all  << std::endl;
