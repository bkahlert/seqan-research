...
//    setDefaultValue(parser,"outdir",);
    ...
    /**
     * set valid outdir, so we don't fail _after_ we read everything
     */
    seqan::getOptionValue(options.outdir, parser, "outdir");
    if(seqan::empty(options.outdir)){
        //options.outdir = options.inputFile
...
//        basec = strdup();
        //dname = dirname(dirc);
        options.outdir = basename(seqan::toCString(options.inputFile));
/*
        int lastindex = options.outdir.find_last_of("."); 
        if(lastindex > 0 ){
            options.outdir = fullname.substr(0, lastindex); 
        }
  */     
    }
