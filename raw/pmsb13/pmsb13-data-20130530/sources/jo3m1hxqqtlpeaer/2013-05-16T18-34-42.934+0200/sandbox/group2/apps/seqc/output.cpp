...
void TSVWriter::guessQualScoreOffset(ReadStats & stats, ScoreInfo & score_info){
    ...
    for(char s = 0; s < 255; ++s){
        std::cout << "checking in score " << (int)s << std::endl;
        for(unsigned pos = 0; pos < stats.size; ++pos){
            if(stats.scoreCount[s][pos]>0){
                if(s < score_info.minScore){
                    std::cout << "was min: " << score_info.minScore << "newly read: " << (int)s << std::endl; 
                    score_info.minScore = s;
                }
                if(s > score_info.maxScore){
                    std::cout << "was max: " << score_info.maxScore << "newly read: " << (int)s << std::endl; 
