...
unsigned sum(seqan::String<unsigned >  counts ){
    unsigned sum = 0; 
    for(unsigned c = 0; c < length(counts)){
        sum += counts[c];
    }
    return sum;
...
void TSVWriter::writeGcContent(ReadStats & stats, std::stringstream & ss)
{
    ...
    for(unsigned pos = 0; pos < length(stats.nucCount); ++pos)
    {
        ...
        seqan::append(counts, stats.nucCount[(seqan::Dna5)'c']);
        seqan::append(counts, stats.nucCount[(seqan::Dna5)'g']);
       
        ss << sum(stats.nucCount[(seqan::Dna5)'c']) << std::endl;
