    ...
    ss << "Base	Mean	Median	Lower Quartile	Upper Quartile	10th Percentile	90th Percentile" 
        << std::endl;
            ...
            for(unsigned pos = 0; pos < length(stats.scoreCount); ++pos){
                ...
                ss << "\t" <<  score_offset - quantiles[9];  // median
                ss << "\t" <<  score_offset - quantiles[4];  // lower quartile
                ss << "\t" <<  score_offset - q quantiles[14];  // upper quartile
                ss << "\t" <<  score_offset - qquantiles[1];  // 10th percentile
                ss << "\t" <<  score_offset - qquantiles[17];  // 90th percentile
                ss << std::endl;
        }
