        ...
        << std::endl;
            ...
            for(unsigned pos = 0; pos < length(stats.scoreCount); ++pos){
                ...
                if(total_scores > 0){                
                    ...
                    ss << "\t" <<  quantiles[9] - stats.score_offset;  // median
                    ss << "\t" <<  quantiles[4] - stats.score_offset;  // lower quartile
                    ss << "\t" <<  quantiles[14] - stats.score_offset;  // upper quartile
                    ss << "\t" <<  quantiles[1] - stats.score_offset;  // 10th percentile
                    ss << "\t" <<  quantiles[17] - stats.score_offset;  // 90th percentile
                    ss << std::endl;
                }
        }
