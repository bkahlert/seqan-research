...
ReadStats::ReadStats(unsigned readLength)
{
    ...
    // can be max = readlength,
    // so we need readlength counters
    seqan::String<unsigned> gc_dist;
    for (unsigned i =0; i<length(stats.nucCount); ++i)
        appendValue(gcPerReadCount,0);
};
