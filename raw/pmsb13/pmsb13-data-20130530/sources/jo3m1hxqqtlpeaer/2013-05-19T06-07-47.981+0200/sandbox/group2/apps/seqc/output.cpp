...
void TSVWriter::guessQualScoreOffset(ReadStats & stats, ScoreInfo & score_info){
    ...
    for(int i=0; i<(sizeof(intervals)/sizeof(*intervals));++i){
        std::cout << intervals[i][0] << " [" << (unsigned)intervals[i][1] << ":" << (unsigned)intervals[i][2]
             << "] contains score range [" << (unsigned)score_info.minScore << ":" << (unsigned)score_info.maxScore 
             << "] ?" << std::endl; 
        if((unsigned)intervals[i][1] >= (unsigned)score_info.minScore){
            std::cout << "contains minScore : " <<
                (unsigned)intervals[i][1] << " >= " << (unsigned)score_info.minScore << std::endl;
        }
        if(intervals[i][2] <= score_info.maxScore){
            std::cout << "contains maxScore" << std::endl;
        }
        if(intervals[i][1] >= score_info.minScore && intervals[i][2] <= score_info.maxScore){
            std::cout << "Yes: " << names[(char)intervals[i][0]] << std::endl;
            seqan::append(possible,names[(char)intervals[i][0]]);
        }
    }
    score_info.description = possible;
