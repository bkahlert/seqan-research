...
/**
 * \brief scoring function for Peptide mode
 ...
 * COMMON_QGRAM_MODE:
 * Counts all 3-grams in the two sequences. The fraction of 3-grams from the smaller sequence which also occur in the bigger sequence is muliplied with 1000 and stored as an integer score value.
 * 
 * Please note that this function is only called on Peptide. For DnaString and RnaString see function GScore.
 */
int GScorePeptideQGram(TScore &score/**<[out] computed score*/, GFastaRecord<TSequence> &newRecord/**<[in] one fasta record*/, GFastaRecord<TSequence> &clusterRecord/**<[in] another fasta record*/,
