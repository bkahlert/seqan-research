...
template<typename TScore,typename TSequence>
/**
 * \brief sorts the entries in a GMatchString using quicksort so that the match with the highest score comes first
 */
int GPostProcessMatches(String<GMatch<TScore,TSequence> > & matches/**<[in/out]object to sort */)
{
	if (length(matches)>=2)
