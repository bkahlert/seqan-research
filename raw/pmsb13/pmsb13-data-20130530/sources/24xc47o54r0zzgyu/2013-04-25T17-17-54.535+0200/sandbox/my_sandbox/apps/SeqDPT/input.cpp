// Program part for parsing the input
#include <iostream>
#include <seqan/sequence.h>
#include <seqan/seq_io.h>

using namespace seqan;

int main(int argc, char const ** argv)
{
	if (argc < 2)
	{
		std::cerr << "Please input path to the input file as first argument.\n";
		return 1;
	}
	
	StringSet<seqan::CharString> ids;
	StringSet<seqan::Dna5String> seqs;
	StringSet<seqan::CharString> quals;
	int res = 0;

	SequenceStream seqStream("D:\Downloads\sample_1.fq");
	//if (!isGood(seqStream))
	//{
	//	std::cerr << "Error while opening the input file.\n";
	//	return 1;
	//}

	//if(!readBatch(ids, seqs, quals, seqStream, 100))
	//{
	//	std::cerr << "Error while reading the sequences.\n";
	//	return 1;
	//}

	return 0;
}