...
int main(int argc, char const ** argv)
{
	...
	typedef Index<TSequence, IndexQGram<Shape<Dna, UngappedShape<qGramLength> >, OpenAddressing> > TIndex;
	...
	while (find(finder, pattern, errorRate, static_cast<int>(minimalLength))) {
	    ...
	    if (length(extendedInfix2) < minimalLength)
		continue;
	    ...
	    outFile << ids1[0] << " range " << beginPosition(extendedInfix1) << "," << endPosition(extendedInfix1) << std::endl;
	    outFile << ids2[0] << " range " << beginPosition(extendedInfix2) << "," << endPosition(extendedInfix2) << std::endl;

	    outFile << globAlign << std::endl;
	    outfile << "----------------------------------------------------------------------------------------------------" << std::endl;
