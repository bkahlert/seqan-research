...
int main(int argc, char const ** argv)
{
	...
	while( find(gollum, the_ring, errorRate, minimalLength) ){
	    ...
	    std::cout << getBeginDim0( wind ) + beginPosition( rows( storm )[0] ) << " - " << endPosition( infix1 ) + endPosition( rows( storm )[0] ) << "\t";
	    std::cout << getBeginDim1( wind ) + beginPosition( rows( storm )[1] ) << " - " << beginPosition( infix2 ) + endPosition( rows( storm )[1] ) << std::endl;
	    outFile << storm;
	}
