...
int main(int argc, char const ** argv)
{
    ...
    if(! isGood(bamStreamIn)){
      std::cerr <<"error: cannot open "<< options.file <<" for reading\n";
      return 1;
    }
    while (!atEnd(bamStreamIn))
    {
        if(readRecord(record, bamStreamIn)!=0){
          std::cerr <<"problems while reading file "<<options.file<<"\n";
          return 1;
        }
	if (hasFlagUnmapped(record))
          lenSum += length(record.seq);
        if(writeRecord(bamStreamOut, record)!=0){
          std::cerr <<"problems while writing to stdout\n";
          return 1;
        }
    }
    std::cout << "length of unmapped reads: " << lenSum << "\n";
    return 0;
}