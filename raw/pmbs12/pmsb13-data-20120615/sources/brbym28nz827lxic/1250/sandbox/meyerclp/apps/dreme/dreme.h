  ...
  for(unsigned i=0;i<=freqL1;++i){
	...
	for(unsigned i=1;i<=freqL1;++i){
		for(unsigned j=1;j<=freqL2;++j){
			...
			M[i][j]=M[i-1][j-1]+ComparePWM(freqMatrix1[i-1],freqMatrix2[j-1],Euklidisch());//je größer, desto unterschiedlicher --> Problem: je weniger abgezogen wird, desto größer
