...
double computeWk(int n,String<Cluster> &cluster){// n=allPWMsLength-n
	...
	***/
	...
	if(clustefr[n].left>=0 && cluster[n].right>=0) //überflüssig wenn mit else anfangen
  ...
  for(unsigned i=0;i<=freqL1;++i){
	...
	for(unsigned i=1;i<=freqL1;++i){
		for(unsigned j=1;j<=freqL2;++j){
			...
			M[i][j]=M[i-1][j-1]+ComparePWM(freqMatrix1[i-1],freqMatrix2[j-1],Pearson());//je größer, desto unterschiedlicher --> Problem: je weniger abgezogen wird, desto größer
