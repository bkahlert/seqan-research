  ...
  for(unsigned i=0;i<=freqL1;++i){
	...
	for(unsigned i=1;i<=freqL1;++i){
		for(unsigned j=1;j<=freqL2;++j){
			...
			if((i==freqL1 && j>freqL2*0.7  && M[i][j]/j<Mmax[0])){//auf die Länge der Überlappung normalisieren --> /j --> über die Hälfte des Kmers soll überlappen
				Mmax[0]=M[i][j]/j;
				Mmax[1]=i;
				Mmax[2]=j;
			}
			else if((j==freqL2 &&i>freqL1*0.7 && M[i][j]/i<Mmax[0])){
				Mmax[0]=M[i][j]/i;
				Mmax[1]=i;
				Mmax[2]=j;
			}
