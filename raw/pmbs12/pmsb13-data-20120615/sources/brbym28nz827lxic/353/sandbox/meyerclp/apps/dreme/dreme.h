...
void loopOverTopKmer(	Seq &seq,String<Iupac> &temp,String<Iupac> &Kmer,unsigned int &pos,Iterator<String<Iupac> >::Type &tempIt,Finder<Index<StringSet<String<Dna5> > > > &finder,unsigned int &counter,std::vector<int> &CounterV,IupacMaps &IMap,std::map<unsigned int,std::map<Iupac,double> > &freqMatrix,String<unsigned int> &replaceString);
...
void loopOverTopKmer(	Seq &seq,
			...
			if((*tempIt == 'A' || *tempIt == 'C' ||*tempIt == 'G' ||*tempIt == 'T')){
				loopOverTopKmer(seq,temp,temp,++posTemp,++tempIt,finder,counter,CounterV,IMaps,freqMatrix,replaceString);//only replace the position with a wildcard
				...
				//	std::cout<<" "<<temp<<" "<<Kmer<<" "<<*replaceIt<<std::endl;
					...
					if(tempIt+1!=end(temp)){
						...
						loopOverTopKmer(seq,temp,temp,++posTemp,++tempIttemp,finder,counter,CounterV,IMaps,freqMatrix,replaceString);
...
void BuildFrequencyMatrix(  std::map<unsigned int,std::map<Iupac,double> > &freqMatrix,
			...
			loopOverTopKmer(seq,temp,Kmer,pos,tempIt,finder,counter,CounterV,IMaps,freqMatrix,replaceString);
