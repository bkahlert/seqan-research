...
double computeDr(Cluster &cluster,std::vector<std::vector<String<double> > > copyCompare){
	...
	double Dr=0;
	std::cout<<"length "<<length(cluster.content)<<std::endl;
	...
	std::cout<<"computeDr "<<Dr<<" "<<Dr/2<<std::endl;
	...
	for(unsigned i=0;i<B;++i){
		...
		for(unsigned k=0;k<allPWMsLength;++k){//addiert die log(Wk's) aller B-Referenz-Daten auf --> nur die Summe wird benötigt
			...
			if(i==0){
				...
				l_quer+=log(cluster[k].Wk)/B;
				l_quer_for_k[k]+=log(cluster[k].Wk)/B;
				std::cout<<"else "<<k<<" "<<l_quer_for_k[k]<<" "<<cluster[k].Wk<<std::endl;
			}
		}
...
void PWMClustering(String< std::map<unsigned int,std::map<Iupac,double> > > &allPWMs,String<Cluster> &cluster){
	...
	***/
	...
	for(unsigned n=allPWMsLength;n>1 && minDifference[0]<2 ;--n){//treshold noch bestimmen --> && minDifference[0]<2
		...
		****/
		if(clusterId[int(minDifference[1])]<0) 
			append(temp,cluster[-clusterId[int(minDifference[1])] - 1],Exact()); 
		...
		****/
		...
		std::cout<<"Dr "<<cluster[allPWMsLength-n].Dr<<std::endl;
		/****
					Wk berechnet
		****/
