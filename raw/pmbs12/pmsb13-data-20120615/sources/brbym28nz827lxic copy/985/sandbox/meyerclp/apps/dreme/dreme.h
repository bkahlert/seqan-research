	...
	for(;tempIt!=end(temp);++tempIt){//loop over each position in kmer
		...
		for(;replaceIt!=end(replace);++replaceIt){// loop over ACGT
			...
			*tempIt=tempChar;//temp wieder das alte, wird aber im nächsten schritt mit einer neuen wildcard ergänzt
			std::cout<<temp<<" "<<*tempIt<<" "<<*replaceIt<<" te "<<IMaps.IupacMapReversed[*tempIt]<<" re "<<IMaps.IupacMapReversed['G']<<" "<<*replaceIt<<" "<<IMaps.IupacMapReversed['C']<<" "<<IMaps.IupacMapReversed['G']<<" "<<IMaps.IupacMapReversed['N']<<" "<<IMaps.IupacMap[IMaps.IupacMapReversed[*tempIt] + IMaps.IupacMapReversed[*replaceIt]];
			system("PAUSE");
