...
SEQAN_DEFINE_TEST(test_PWMClustering){
	...
	*****/
	...
	/*for (j = int(minDifference[1])+1; j < int(minDifference[2]); j++)
		compare[int(minDifference[1])][j] = std::max(compare[j][int(minDifference[2])],compare[int(minDifference[1])][j]);
	...
	for (j = 0; j < int(minDifference[2]); j++) compare[j][int(minDifference[2])] = compare[j][n-1];
	for (j = int(minDifference[2])+1; j < n-1; j++) compare[int(minDifference[2])][j] = compare[j][n-1];
*/
	/*****
			New Matrix:
