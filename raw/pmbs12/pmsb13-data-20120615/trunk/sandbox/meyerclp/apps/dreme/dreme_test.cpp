...
SEQAN_DEFINE_TEST(test_ChIPSeq_generalize){
	...
	*/
	...
	//Kmer zum rübergehen, Temp zum neubefüllen
...
SEQAN_DEFINE_TEST(test_ChIPSeq_BuildFrequency){
	...
	SEQAN_ASSERT_EQ(double(int((seq.freqMatrix[1]['A']+0.005)*100))/100,0.06);
	SEQAN_ASSERT_EQ(double(int((seq.freqMatrix[1]['G']+0.005)*100))/100,0.49);//in der hälfte der sequenzen kommt ACG und AGG vor, ACG und AGG können in der selben sequenz sein
	SEQAN_ASSERT_EQ(double(int((seq.freqMatrix[1]['C']+0.005)*100))/100,0.42);
	SEQAN_ASSERT_EQ(double(int((seq.freqMatrix[2]['G']+0.005)*100))/100,0.89);
